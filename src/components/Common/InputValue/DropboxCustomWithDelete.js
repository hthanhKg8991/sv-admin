...
class DropboxCustomWithDelete extends Component {
    constructor(props) {
        super(props);
        this.state = {
            lang: props.lang,
    ...
    componentWillReceiveProps(newProps) {
        ...
        if (data?.length){
            ...
            data.forEach((item) => {
                ...
                const itemObj = JSON.parse(item[key_value]);
                
                if (_.isEqual(valObj, itemObj) && isShowIconDelete !== true) {
                    isShowIconDelete = true;
    ...
    componentDidMount(){
        ...
        if(value && data?.length){
            const item = data.find((obj) => {
                const valObj = JSON.parse(value);
                const itemObj = JSON.parse(obj.value);

                return _.isEqual(valObj, itemObj)
            })

            this.setState({value:item?.value,title:item?.title})
        }
    }
    ...
    render() {
        ...
        if (this.state.list_item){
            this.state.list_item.forEach((item) => {
                data.push({label: item[this.state.key_title], value: item[this.state.key_value], id: item?.id, isDisabled: item.isDisabled || false});
        ...
        return(
            <div className="v-input">
                <div className={classnames(
                    ...
                    <div className="v-input-slot custom-dropbox-with-delete-outer-contain">
                        ...
                        {!this.props.readOnly && (
                            <ReactSelect
                                ...
                                isOptionSelected={(obj) => {
                                    const {idField, value} = this.props

                                    const valObj = JSON.parse(value);
                                    const itemObj = JSON.parse(obj.value);

                                    return valObj[idField] == itemObj[idField]
                                ...
                                components={{
                                    ...
                                    Option: (selProp) => {
                                        return (
                                            <div className={`d-flex custom-filter custom-filter-select-item ${selProp.isSelected && "selected-item"}`} style={{padding:"8px 12px"}}>
                                                <div {...selProp?.innerProps} className="bg-transparent w-full text-overflow-ellip" style={{width:"100%"}}>
                                                    {selProp?.data?.label}
                                                </div>
                                                <div 
                                                    className="delete-filter" style={{zIndex:200}} 
                                                    onClick={(e) => {
                                                        e.preventDefault();
                                                        e.stopPropagation();
                                                        this.props.onDeleteFunc(selProp?.data?.id)
                                                    }}
                                                >
                                                    x
                                                </div>
...
export default connect(mapStateToProps,mapDispatchToProps)(DropboxCustomWithDelete);
