...
class Gird extends React.Component {
    ...
    async asyncData(params = {}) {
        const {defaultQuery, fetchApi, history, query, isPushRoute, onFetchSuccess, isReplaceRoute, commitCV, isMustHaveID} = this.props;
        ...
        if (data) {
            this.setState({
                loading: false,
                data: commitCV ? [..._.get(data, ['province_ids'], []),..._.get(data, ['conditions'], [])] : _.get(data, ['items'], []),
                perPage: perPage,
                filter: filter,
                pagination: {
...
Gird.propTypes = {
    ...
    isMustHaveID: PropTypes.bool, // Loại trừ cho 1 số api get List bắt buộc truyền id
    ...
    commitCV: PropTypes.bool, // commitCV sẽ trả về kiểu dữ liệu khác vì update multi
};

export default connect(mapStateToProps, null)(Gird);
