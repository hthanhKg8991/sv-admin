...
class TableComponent  extends Component {
    ...
    componentWillReceiveProps(newProps) {
        ...
        if (newProps.children){
            ...
            if(Array.isArray(newProps.children)) {
                newProps.children.forEach(item => {
    ...
    render () {
        let {children, select, dragging} = this.state;
        let TableHeader = [];
        let TableBody = [];
        if(Array.isArray(children)) {
            TableHeader = children.filter(c => c.props && c.props.tableType === "TableHeader");
            TableBody = children.filter(c => c.props && c.props.tableType === "TableBody");
        }
        ...
        return (
            <DragScroll allow={this.props.allowDragScroll} allowSelect={false} height="100%" width="100%" onChange={(select, dragging)=>{
