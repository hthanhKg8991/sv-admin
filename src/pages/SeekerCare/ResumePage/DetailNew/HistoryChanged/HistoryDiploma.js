                ...
                <TableBody tableType="TableBody">
                    {data_list.map((item, key)=> {
                        let data_change = {
                            ...
                            start_end_date: item?.start_date && item?.end_date ? `${moment.unix(item?.start_date).format("MM/YYYY")} - ${moment.unix(item?.end_date).format("MM/YYYY")}` : undefined,
                            img_diploma_url: certificate_rate[item?.img_diploma_url] ? certificate_rate[item?.img_diploma_url] : item?.img_diploma_url,
                        ...
                        let old_data = {
                            ...
                            start_end_date: item?.old_data?.start_date && item?.old_data?.end_date ? `${moment.unix(item?.old_data?.start_date).format("MM/YYYY")} - ${moment.unix(item?.old_data?.end_date).format("MM/YYYY")}` : undefined,
                            img_diploma_url: certificate_rate[item?.old_data?.img_diploma_url] ? certificate_rate[item?.old_data?.img_diploma_url] : item?.old_data?.img_diploma_url,
                            ...
                            <React.Fragment key={key}>
                                <tr className={classnames("el-table-row pointer", key % 2 !== 0 ? "tr-background" : "", String(itemActive) === String(key) ? 'active' : '')} onClick={()=>{this.activeItem(key)}}>
                                    {Object.keys(old_data).map((name, key) => {
                                        ...
                                        if(name === "img_diploma_url" && value){
                                            return(
                                                <td key={key}>
                                                    <div className={classnames("cell", cl)} title={value}>
                                                        <a href={value} target="_blank" rel="noopener noreferrer">Xem file</a>
                                                    </div>
                                                </td>
                                            )
                                        }
                                ...
                                {String(itemActive) === String(key) && (
                                    <tr className={classnames("el-table-item")} onClick={()=>{this.activeItem(key)}}>
                                        {Object.keys(old_data).map((name, key) => {
                                            if(name === "img_diploma_url" && old_data[name]){
                                                return(
                                                    <td key={key}>
                                                        <div className="cell" title={old_data[name]}>
                                                            <a href={old_data[name]} target="_blank" rel="noopener noreferrer">Xem file</a>
                                                        </div>
                                                    </td>
                                                )
                                            }
