...
class HistoryDetail extends Component {
    ...
    _btnReject(){
        ...
        if (!reject.rejected_reason.length && !reject.rejected_reason_note) {
            ...
            let args = {
                id: this.props.resume_id,
                rejected_reason: reject.rejected_reason,
                rejected_reason_note: reject.rejected_reason_note
            };
            this.setState({loading: true},()=>{
                this.props.apiAction.requestApi(apiFn.fnPost, config.apiResumeDomain, ConstantURL.API_URL_POST_RESUME_REVISION_REJECT, args);
            });
        }
    }
    _btnApprove(){
        this.setState({loading: true},()=>{
            this.props.apiAction.requestApi(apiFn.fnPost, config.apiResumeDomain, ConstantURL.API_URL_POST_RESUME_REVISION_APPROVE, {id: this.props.resume_id});
        });
    ...
    componentWillReceiveProps(newProps) {
        ...
        if (newProps.api[ConstantURL.API_URL_POST_RESUME_REVISION_APPROVE]){
            this.setState({loading: false});
            let response = newProps.api[ConstantURL.API_URL_POST_RESUME_REVISION_APPROVE];
            if (response.code === Constant.CODE_SUCCESS) {
            ...
            this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_RESUME_REVISION_APPROVE);
        }
        if (newProps.api[ConstantURL.API_URL_POST_RESUME_REVISION_REJECT]){
            this.setState({loading: false});
            let response = newProps.api[ConstantURL.API_URL_POST_RESUME_REVISION_REJECT];
            if (response.code === Constant.CODE_SUCCESS) {
                this.props.uiAction.putToastSuccess("Thao tác thành công!");
                this.props.uiAction.deletePopup();
                this.props.uiAction.refreshList('ResumePage', {delay: Constant.DELAY_LOAD_LIST_2S});
            ...
            this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_RESUME_REVISION_REJECT);
        }
    }
    ...
    render () {
        ...
        let mapping = {
            ...
            occupation_ids:{
                Content: (key)=>{
                    if (key) {
                        let data_arr = this.props.sys.occupations.items.filter(c => key.includes(c.id));
                        let data_string = '';
                        data_arr.forEach((item, key) => {
                            data_string += key === 0 ? item.name : ', ' + item.name;
                        });
            ...
            level:{
                Content: (key)=>{
                    let data = utils.convertObjectValueCommonData(this.props.sys.common.items,Constant.COMMON_DATA_KEY_seeker_level);
                    return data[key] ? data[key] : key;
                }
            },
            revision_status:{
        ...
        console.log(data_list, "name: data_list.lg")
        return (
            <div className="relative form-container">
                            ...
                            <TableComponent allowDragScroll={false}>
                                ...
                                <TableBody tableType="TableBody">
                                    {Array.isArray(data_list) && data_list.map((item,key)=> {
                                        ...
                                        console.log(old_data, "name: old_data.lg")
                                        ...
                                        return(
                                            <tr key={key} className={classnames("el-table-row pointer", (key % 2 !== 0 ? "tr-background" : ""))}>
                                                ...
                                                <td>
                                                    <div className="cell" title={old_data}>
                                                        <div dangerouslySetInnerHTML={{__html: old_data}} />
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className="cell" title={old_new}>
                                                        <div dangerouslySetInnerHTML={{__html: old_new}} />
                                                    </div>
                                                </td>
                                            </tr>
                                        )
                            ...
                            <div className="box-inf">
                                <div className="nav-box">
                                    <div className="nav-group">
                                        <div className={classnames("nav-item pointer", navActive === 1 ? "active" : "")} onClick={()=>{this.onChangeNav(1)}}>Kinh nghiệm</div>
                                        <div className={classnames("nav-item pointer", navActive === 2 ? "active" : "")} onClick={()=>{this.onChangeNav(2)}}>Bằng cấp</div>
                                        <div className={classnames("nav-item pointer", navActive === 3 ? "active" : "")} onClick={()=>{this.onChangeNav(3)}}>Ngoại ngữ</div>
                                        <div className={classnames("nav-item pointer", navActive === 4 ? "active" : "")} onClick={()=>{this.onChangeNav(4)}}>Tin Học</div>
                                        <div className={classnames("nav-item pointer", navActive === 5 ? "active" : "")} onClick={()=>{this.onChangeNav(5)}}>Kỹ năng sở trường</div>
                                        <div className={classnames("nav-item pointer", navActive === 6 ? "active" : "")} onClick={()=>{this.onChangeNav(6)}}>Người tham khảo</div>
                                    </div>
                                </div>
                ...
                {this.props.object.revision_status === Constant.STATUS_INACTIVED && <div className="crm-section" style={{padding:"5px 20px"}}>
                    {this.props.resume_id  &&
                    (
                        <div>
