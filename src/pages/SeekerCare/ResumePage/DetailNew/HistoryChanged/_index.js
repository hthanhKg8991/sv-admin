...
class _index extends Component {
    ...
    _refreshList(){
        let args = {
            resume_id: this.props.object.id,
            per_page: this.state.per_page,
            page: this.state.page
        };

        this.setState({loading: true},()=>{
            this.props.apiAction.requestApi(apiFn.fnGet, config.apiSeekerDomain, ConstantURL.API_URL_GET_RESUME_REVISION_LIST, args);
        });
    }
    _activeItem(key){
    ...
    componentWillReceiveProps(newProps) {
        if (newProps.api[ConstantURL.API_URL_GET_RESUME_REVISION_LIST]){
            let response = newProps.api[ConstantURL.API_URL_GET_RESUME_REVISION_LIST];
            ...
            this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_GET_RESUME_REVISION_LIST);
                                ...
                                <TableBody tableType="TableBody">
                                    {this.state.data_list.map((item, key)=> {
                                        ...
                                        item.rejected_reason.forEach((reason) => {
                                            rejected_reason += '- ' + resume_rejected_reason[reason] + "\n";
                                        });
                                        rejected_reason = item.rejected_reason_note ? rejected_reason + '- ' + item.rejected_reason_note : rejected_reason;

                                        let i = {
                                            updated_by: item.updated_by,
                                            updated_at: moment.unix(updated_at).format('DD/MM/YYYY HH:mm:ss'),
                                            revision_status: seeker_status[item.revision_status],
                                            approved_by: item.approved_by,
                                            approved_at: [Constant.STATUS_ACTIVED, Constant.STATUS_DISABLED].includes(parseInt(item.revision_status)) ?  moment.unix(updated_at).format('DD/MM/YYYY') : '',
