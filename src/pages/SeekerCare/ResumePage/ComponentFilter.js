...
class ComponentFilter extends Component {
  ...
  render() {
    ...
    let resume_status = utils.convertArrayValueCommonData(this.props.sys.common.items, Constant.COMMON_DATA_KEY_resume_status);
    let seeker_has_mobile_status = utils.convertArrayValueCommonData(
      this.props.sys.common.items,
      Constant.COMMON_DATA_KEY_resume_has_mobile_status
    );
    ...
    const resume_is_search_allowed = utils.convertArrayValueCommonData(
      this.props.sys.common.items,
      Constant.COMMON_DATA_KEY_is_search_allowed
    );
    const is_applied_status_new = utils.convertArrayValueCommonData(
      this.props.sys.common.items,
      Constant.COMMON_DATA_KEY_is_applied_status_new
    );

    let province = this.props.sys.province.items;
    const { query, menuCode, idKey, is_archived } = this.props;
    const old_channel_code = utils.convertArrayValueCommonData(
      this.props.sys.common.items,
      Constant.COMMON_DATA_KEY_old_channel_code_employer
    );
    ...
    return (
      <FilterLeft idKey={idKey} query={query} menuCode={menuCode}>
        <SearchField type="input" label="Tiêu đề, Mã hồ sơ" name="q" timeOut={1000} />
        <SearchField type="input" label="ID NTV, tên NTV, email" name="seeker_q" timeOut={1000} />
        <SearchField type="dropbox" label="Trạng thái số điện thoại" name="seeker_has_mobile" data={seeker_has_mobile_status} />
        <SearchField type="dropbox" label="Trạng thái hồ sơ" name="status" data={resume_status} />
        <SearchField type="dropbox" label="Trạng thái hiển thị" name="is_search_allowed" data={resume_is_search_allowed} />
        <SearchField type="dropbox" label="Trạng thái ứng tuyển" name="is_applied_status_new" data={is_applied_status_new} />
