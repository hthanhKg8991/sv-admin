                ...
                <TableBody tableType="TableBody">
                    {data_list.map((item, key)=> {
                        let data_change = {
                            ...
                            salary: utils.formatNumber(item.salary, 0, ".", ""),
                            salary_unit: currencyList[item.salary_unit] ? currencyList[item.salary_unit].name : item.salary_unit,
                            start_end_date: item.start_date ? `${moment.unix(item.start_date).format("MM/YYYY")} - ${item.is_current_work === 1 ? 'Hiện tại' : moment.unix(item.end_date).format("MM/YYYY")}` : '',
                        ...
                        let old_data = {
                            ...
                            salary: utils.formatNumber(item.old_data.salary, 0, ".", ""),
                            salary_unit: currencyList[item.old_data.salary_unit] ? currencyList[item.old_data.salary_unit].name : item.old_data.salary_unit,
                            start_end_date: item.old_data.start_date ? `${moment.unix(item.old_data.start_date).format("MM/YYYY")} - ${item.old_data.is_current_work === 1 ? 'Hiện tại' : moment.unix(item.old_data.end_date).format("MM/YYYY")}` : '',
                            description: item.old_data.description,
                            achieved: item.old_data.achieved
                        };
                        return(
