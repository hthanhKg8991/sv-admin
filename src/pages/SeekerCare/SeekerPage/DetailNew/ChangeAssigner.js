...
class PopupChangeBasket extends Component {
  ...
  _refreshList(delay = 0){
    ...
    this.props.apiAction.requestApi(apiFn.fnGet, config.apiEmployerDomain, ConstantURL.API_URL_GET_LIST_EMPLOYER_HISTORY_CHANGE_BASKET, args, delay);
  }
  _hidePopup(){
  ...
  _onSave(event){
    ...
    this.props.apiAction.requestApi(apiFn.fnPost, config.apiEmployerDomain, ConstantURL.API_URL_POST_CHANGE_BASKED_EMPLOYER, basket);
  }
  componentWillMount(){
    this.refreshList();
  ...
  componentWillReceiveProps(newProps) {
    if (newProps.api[ConstantURL.API_URL_GET_LIST_EMPLOYER_HISTORY_CHANGE_BASKET]){
      let response = newProps.api[ConstantURL.API_URL_GET_LIST_EMPLOYER_HISTORY_CHANGE_BASKET];
      if (response.code === Constant.CODE_SUCCESS) {
        this.setState({data_list: response.data.items});
        this.setState({pagination_data: response.data});
      }
      this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_GET_LIST_EMPLOYER_HISTORY_CHANGE_BASKET);
    }
    ...
    if (newProps.api[ConstantURL.API_URL_POST_CHANGE_BASKED_EMPLOYER]){
      let response = newProps.api[ConstantURL.API_URL_POST_CHANGE_BASKED_EMPLOYER];
      if (response.code === Constant.CODE_SUCCESS) {
      ...
      this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_CHANGE_BASKED_EMPLOYER);
    }
  }
  ...
  render () {
    ...
    let employer_discharged_reason_arr = utils.convertArrayValueCommonData(this.props.sys.common.items,Constant.COMMON_DATA_KEY_employer_discharged_reason);
          ...
          <div className="popupContainer">
            <div className="form-container row">
              <form onSubmit={this.onSave}>
                ...
                <div className="col-sm-6 col-xs-12 mb15">
                  <DropboxMulti name="reason" label="Lý do chuyển" data={employer_discharged_reason_arr}
