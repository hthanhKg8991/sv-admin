...
import Detail from "pages/SeekerCare/SeekerPage/DetailNew/HistoryChanged/Detail";
import {seekerListRevision} from "api/seeker";
...
class HistoryChanged extends React.Component {
            ...
            columns: [
                ...
                },
                {
                    ...
                    cell: row => (
                        row.approved_at &&
                        <React.Fragment>
                            {_.has(row, 'approved_at') && moment.unix(row.approved_at).format("DD/MM/YYYY")}
                        </React.Fragment>
                    )
                },
                {
    ...
    _expandRow(row) {
        const {history} = this.props;
        const {id} = this.state;
        return (<Detail detailId={row.id} seekerId={id} history={history}/>);
    }
    ...
    render() {
        ...
        return (
            <Default title={'Lịch sử thay đổi NTV'}
                     titleActions={(
                ...
                <div className={"row mt15"}>
                    ...
                    <div className={"col-md-12 mt10"}>
                        <Gird idKey={"HistoryChanged"}
                              fetchApi={seekerListRevision}
                              defaultQuery={{seeker_id: id}}
                              columns={columns}
                              history={history}
                              isPushRoute={false}
