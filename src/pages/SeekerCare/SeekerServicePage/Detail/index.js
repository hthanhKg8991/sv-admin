...
class index extends Component {
    ...
    _btnEdit(){
        this.props.uiAction.createPopup(PopupCustomerService, "Chỉnh Sửa Nhóm CSNTV", {object: this.props.item});
    ...
    _btnAddMember(item){
        this.props.uiAction.createPopup(PopupAddMember, "Thêm CSNTV Vào Nhóm", {
            object: {team_id: this.state.item.id},
            branch_code_real: item.branch_code
        });
    }
                    ...
                    <div className="row">
                        <div className="col-sm-12 col-xs-12 crm-section">
                            <div className="top-table">
                                <div className="left">
                                    <button type="button" className="el-button el-button-primary el-button-small" onClick={() => this.btnAddMember(item)}>
                                        <span>Thêm CSNTV <i className="glyphicon glyphicon-plus"/></span>
                                    </button>
                                </div>
                            </div>
                            {loading ? (
                                        ...
                                        {data_list.map((member, key)=> {
                                            ...
                                            return(
                                                <tr key={key} className={classnames("el-table-row", key % 2 !== 0 ? "tr-background" : "")}>
                                                    ...
                                                    <td>
                                                        <div className="cell">
                                                            <SpanCommon idKey={Constant.COMMON_DATA_KEY_division_status} value={member?.staff_status}/>
                                                        </div>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <div className="cell">
                                                            <div className="text-underline pointer">
                                                                {member?.staff_status !== Constant.STAFF_LOCK && <span className="text-bold text-primary" onClick={()=>{this.btnEditMember(item)}}>Chỉnh sửa</span>}
                                                                <span className="ml5 text-bold text-danger" onClick={()=>{this.btnDeleteMember(member)}}>Xóa</span>
                                                            </div>
                                                        </div>
                                                    </td>
                                                </tr>
