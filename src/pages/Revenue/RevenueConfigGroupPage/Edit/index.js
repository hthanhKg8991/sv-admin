...
import {createConfigGroup, getDetailConfigGroup, updateConfigGroup,} from "api/commission";
...
class Edit extends React.Component {
    ...
    async submitData(data, setErrors) {
        ...
        if (id > 0) {
            data.id = id;
            res = await updateConfigGroup(data);
        } else {
            res = await createConfigGroup(data);
        }
        if (res) {
    ...
    async asyncData() {
        ...
        if (id > 0) {
            const res = await getDetailConfigGroup({id});
            if (res) {
    ...
    render() {
        ...
        const validationSchema = Yup.object().shape({
            ...
            ordering: Yup.number().required(Constant.MSG_REQUIRED),
        });
