...
import {createKPIConfig, getDetailKPIConfig, updateKPIConfig} from "api/commission";
...
class Edit extends React.Component {
    ...
    _goBack(id) {
        ...
        if (id > 0) {
            if (_.get(history, 'action') === 'POP') {
                history.push({
                    pathname: Constant.BASE_URL_REVENUE_CONFIG,
                    search: '?action=list'
                });
    ...
    async submitData(data, setErrors) {
        ...
        if (id > 0) {
            data.id = id;
            res = await updateKPIConfig(data);
        } else {
            res = await createKPIConfig(data);
        }
        if (res) {
    ...
    async asyncData() {
        ...
        if (id > 0) {
            const res = await getDetailKPIConfig({id});
            if (res) {
    ...
    render() {
        ...
        const validationSchema = Yup.object().shape({
            ...
            start_at: Yup.string().required(Constant.MSG_REQUIRED),
            end_at: Yup.string().required(Constant.MSG_REQUIRED),
            detail_configs: Yup.array().of(
                Yup.object().shape({
                    room_id: Yup.string().required(Constant.MSG_REQUIRED),
                    division_code: Yup.string().required(Constant.MSG_REQUIRED),
                    rating: Yup.string().required(Constant.MSG_REQUIRED),
                    level: Yup.string().required(Constant.MSG_REQUIRED),
        ...
        dataForm.detail_configs = dataForm?.detail_configs?.length > 0 ? dataForm.detail_configs : [Constant.CONDITION_CONFIG_KPI];
