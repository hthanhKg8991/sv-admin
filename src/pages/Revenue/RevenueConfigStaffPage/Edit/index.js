...
import {createConfigStaff, getDetailConfigStaff, updateConfigStaff,} from "api/commission";
...
class Edit extends React.Component {
    ...
    async submitData(data, setErrors) {
        ...
        if (id > 0) {
            data.id = id;
            res = await updateConfigStaff(data);
        } else {
            res = await createConfigStaff(data);
        }
        if (res) {
    ...
    async asyncData() {
        ...
        if (id > 0) {
            const res = await getDetailConfigStaff({id});
            if (res) {
    ...
    render() {
        ...
        const validationSchema = Yup.object().shape({
            ...
            group_code: Yup.string().required(Constant.MSG_REQUIRED),
            config_id: Yup.number().required(Constant.MSG_REQUIRED),
            position: Yup.number().required(Constant.MSG_REQUIRED),
        });
                ...
                <FormBase onSubmit={this.onSubmit}
                    ...
                    <div className={"row mt15"}>
                        <div className="col-sm-12">
                            <CanRender actionCode={ROLES.revenue_config_staff_update}>
