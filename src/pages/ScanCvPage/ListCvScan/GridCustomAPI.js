...
class Gird extends React.Component {
    ...
    async asyncData(params = {}) {
        const {defaultQuery, fetchApi, history, query, isPushRoute, onFetchSuccess, isReplaceRoute, commitCV} = this.props;
        ...
        if (data) {
            this.setState({
                ...
                filter: filter,
                pagination: {
                    pageCurrent: parseInt(_.get(data, 'meta.page', 0)),
                    totalPage: _.get(data, 'meta.pageCount', 0),
                    totalItem: _.get(data, 'meta.total', 0)
                }
            });
            //push router
            if (isPushRoute) {
