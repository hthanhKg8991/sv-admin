...
class TableHeader  extends Component {
    ...
    _onResize(event,{size}){
        let length = this.state.maxLenght * 10;
        this.setState({width: size.width <= length ? length : size.width});
    }
    componentWillMount() {
        ...
        if (!!children && typeof children === 'string' || children instanceof String) {
            ...
            this.setState({maxLenght: max});
        }else{
            this.setState({maxLenght: 5});
        }
    }
