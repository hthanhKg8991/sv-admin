...
class GirdCustomHeader extends React.Component {
    ...
    async asyncData(params = {}) {
        const {defaultQuery, fetchApi, history, query, isPushRoute, isReplaceRoute, requireParam, isRequireParam} = this.props;
        ...
        if(isRequireParam && !paramFull[requireParam] ){
            this.setState({
                loading: false,
                data: []
            });
            ...
            this.setState({
                loading: false,
                data: Array.isArray(data) ? data : _.get(data, ['items'], []),
                perPage: perPage,
                filter: filter,
                pagination: {
    ...
    componentWillReceiveProps(newProps) {
        const {idKey, onResetSelection} = this.props;
    ...
    _renderTableHeader() {
        ...
        return columns(data).map((column, index) => (
    ...
    _renderRow(item, key) {
        const {columns, indexExpandRow, data} = this.state;
...
GirdCustomHeader.propTypes = {
    isRequireParam: PropTypes.bool,
    requireParam: PropTypes.any,
