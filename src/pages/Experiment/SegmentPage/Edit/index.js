...
import {createSegment, getDetailSegment, updateSegment} from "api/audience";
...
class Edit extends React.Component {
    ...
    async submitData(data, setErrors) {
        ...
        if (id > 0) {
            data.id = id;
            res = await updateSegment(data);
        } else {
            res = await createSegment(data);
        }
        if (res) {
    ...
    async asyncData() {
        ...
        if (id > 0) {
            const res = await getDetailSegment({id});
            if (res) {
    ...
    render() {
        ...
        const validationSchema = Yup.object().shape({
            ...
            ring: Yup.string().required(Constant.MSG_REQUIRED),
