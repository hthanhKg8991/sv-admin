...
import {createFeatureFlag, getDetailFeatureFlag, updateFeatureFlag} from "api/featureFlag";
...
class Edit extends React.Component {
    ...
    async submitData(data, setErrors) {
        ...
        if (id > 0) {
            data.id = id;
            res = await updateFeatureFlag(data);
        } else {
            res = await createFeatureFlag(data);
        }
        if (res) {
    ...
    async asyncData() {
        ...
        if (id > 0) {
            const res = await getDetailFeatureFlag({id});
            if (res) {
