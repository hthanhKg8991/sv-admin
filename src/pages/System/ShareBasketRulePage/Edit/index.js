...
import {
    updateShareBasketRule,
    createShareBasketRule,
    getDetailShareBasketRule
} from "api/employer";
...
class Edit extends React.Component {
    ...
    async submitData(data, setErrors) {
        ...
        if (id > 0) {
            data.id = id;
            res = await updateShareBasketRule(data);
        } else {
            res = await createShareBasketRule(data);
        }
        if (res) {
    ...
    async asyncData() {
        ...
        if (id > 0) {
            const res = await getDetailShareBasketRule({id});
            if (res) {
    ...
    render() {
        ...
        const validationSchema = Yup.object().shape({
            ...
            type: Yup.string().required(Constant.MSG_REQUIRED).nullable(),
            company_kind: Yup.string().nullable(),
            throwout_type: Yup.string().nullable(),
