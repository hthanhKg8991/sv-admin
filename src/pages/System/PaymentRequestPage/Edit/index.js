...
import {createPaymentRequest, getDetailPaymentRequest, updatePaymentRequest} from "api/saleOrder";
...
class Edit extends React.Component {
    ...
    async submitData(data, setErrors) {
        ...
        if (id > 0) {
            data.id = id;
            res = await updatePaymentRequest(data);
        } else {
            res = await createPaymentRequest(data);
        }
        if (res) {
    ...
    async asyncData() {
        ...
        if (id > 0) {
            const res = await getDetailPaymentRequest({id});
            if (res) {
