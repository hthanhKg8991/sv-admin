...
import {getList, rejectV3} from "api/booking";
import {publish} from "utils/event";
import Default from "components/Layout/Page/Default";
import WrapFilter from "components/Common/Ui/Table/WrapFilter";
import ComponentFilter from "pages/CustomerCare/BookingPage/ComponentFilter";
...
import FormReject from "pages/CustomerCare/BookingPage/FormReject";
...
class List extends Component {
            ...
            columns: [
                {
                    ...
                    cell: row => (
                        <Link
                            to={`${Constant.BASE_URL_BOOKING}?${queryString.stringify({...paramsQuery, ...{code: row.code}})}`}>
                            <span>{row.code}</span>
                        </Link>
                    )
                },
    ...
    async _onRejectSuccess(res, data) {
        ...
        if(res?.code === Constant.CODE_RES_CONFIRM) {
            ...
            if(confirm) {
                const res_continue = await rejectV3({...data, allowed_continue: true});
    ...
    render() {
        ...
        return (
            <Default
                ...
                buttons={
                    <CanRender actionCode={ROLES.customer_care_booking_management_create}>
                ...
                <Gird idKey={idKey} fetchApi={getList}
                ...
                <PopupForm onRef={ref => (this.popupReject = ref)}
                           ...
                           initialValues={{cancelled_reason: '', reason_note: ''}}
