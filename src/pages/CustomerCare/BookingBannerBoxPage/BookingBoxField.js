...
class BookingBoxField extends Component {
    ...
    _refreshList(delay = 0) {
        ...
        this.props.apiAction.requestApi(apiFn.fnGet, config.apiBookingDomain, ConstantURL.API_URL_GET_LIST_BOOKING_BANNER_BOX_FIELD, param_search, delay);
    ...
    _onChangeGate(value, name) {
        ...
        this.setState({param_search: {gate_code: value}, job_field: job_field});
    }


    componentWillReceiveProps(newProps) {
        if (newProps.api[ConstantURL.API_URL_GET_LIST_BOOKING_BANNER_BOX_FIELD]) {
            let response = newProps.api[ConstantURL.API_URL_GET_LIST_BOOKING_BANNER_BOX_FIELD];
            if (response.code === Constant.CODE_SUCCESS) {
                this.setState({data_list: response.data});
            }
            this.setState({loading: false});
            this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_GET_LIST_BOOKING_BANNER_BOX_FIELD);
        }
    }
            ...
            <div className="card-body">
                <div className="sub-title-form crm-section inline-block">
                    <div className={classnames('pointer', show_detail ? 'active' : '')} onClick={this.showDetail}>
                        Trang ngành <i aria-hidden="true" className="v-icon material-icons v-icon-append"
