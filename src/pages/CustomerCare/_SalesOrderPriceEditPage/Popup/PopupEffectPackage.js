...
class PopupEffectPackage extends Component {
    constructor(props) {
        ...
        this.state = {
            object: {},
            object_required: ['parent_service_code', 'service_code', 'job_quantity', 'start_date', 'week_quantity'],
            object_error: {},
            name_focus: "",
            package_running: []
        };
    ...
    async _onChangeService(value, name) {
        ...
        if (value && object.parent_service_code) {
            const res = await getDetailSKU({service_code: object.parent_service_code, effect_code: value});
            if (res) {
