...
import PopupJobBasic from '../Popup/PopupJobBasic';
...
class JobBasicPackage extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: true,
    ...
    _btnBuy(name){
        this.props.uiAction.createPopup(PopupJobBasic, "Mua "+name, {sales_order: this.props.sales_order});
    }
    ...
    _btnEdit(object,name){
        this.props.uiAction.createPopup(PopupJobBasic, "Chỉnh Sửa Mua "+name, {
    ...
    componentWillReceiveProps(newProps) {
        if (newProps.api[ConstantURL.API_URL_GET_SALES_ORDER_ITEMS_LIST]){
            ...
            if (response.info?.args?.service_type === Constant.SERVICE_TYPE_JOB_BASIC) {
        ...
        if (newProps.refresh['JobBasicPackage']){
            let delay = newProps.refresh['JobBasicPackage'].delay ? newProps.refresh['JobBasicPackage'].delay : 0;
            this.refreshList(delay);
            this.props.uiAction.deleteRefreshList('JobBasicPackage');
        }
    }
    ...
    render () {
        ...
        let {sales_order,sys} = this.props;
            ...
            <div className="col-result-full crm-section">
                <div className="box-card box-full">
                    <div className="box-card-title pointer box-package" onClick={this.showHide}>
                        <span className="title left">{detailService?.display_name_frontend || detailService?.display_name_contract} ({data_list.length})</span>
                            ...
                            {this.state.loading ? (
                                ...
                                <div className="card-body">
                                    {parseInt(sales_order.status) !== Constant.STATUS_DELETED &&(
                                        <div className="left">
                                            <button type="button" className="el-button el-button-primary el-button-small" onClick={()=>this.btnBuy(detailService?.display_name_frontend || detailService?.display_name_contract)}>
                                                <span>Mua {detailService?.display_name_frontend || detailService?.display_name_contract} <i className="glyphicon glyphicon-plus"/></span>
                                            </button>
                                        </div>
                                    )}
                                    <div className="right">
                                        ...
                                        <div className="body-table el-table">
                                            <TableComponent className="table-custom">
                                                <TableHeader tableType="TableHeader" width={300}>
                                                    Thông tin mua {detailService?.display_name_frontend || detailService?.display_name_contract}
                                                </TableHeader>
                                                <TableHeader tableType="TableHeader" width={300}>
                                                    ...
                                                    {data_list.map((item, key)=> {
                                                        return (
                                                            <tr key={key} className={classnames("el-table-row", (key % 2 !== 0 ? "tr-background" : ""))}>
                                                                ...
                                                                <td>
                                                                    <div className="cell-custom">
                                                                        {item.items.length > 0 && item.items.map((effect,key_effect) => {
                                                                            return (
                                                                                <div key={key_effect} >
                                                                                    <div>Gói làm mới: <span className="text-bold">{effect.cache_service_name}</span></div>
                                                                                    <div>Thời gian mua: <span className="text-bold">
                                                                            ...
                                                                            if (parseInt(price) === displayed_area || displayed_area === Constant.AREA_ALL) {
                                                                                return (
                                                                                    <div key={key_price} className="mb10-child">
                                                                                        ...
                                                                                        <div>Giá niêm yết: <span className="text-bold">{utils.formatNumber(item.price_list_info[price].based_price, 0, ".", "đ")} ({"1 tin / " + utils.formatNumber(item.price_list_info[price].unit_per_based, 0, ".", "ngày")})</span></div>
                                                                ...
                                                                <td>
                                                                    <div className="cell-custom">
                                                                        <div>Thành tiền {detailService?.display_name_frontend || detailService?.display_name_contract}: <span className="text-bold">{utils.formatNumber(item.original_amount,0,".","đ")}</span></div>
                                                                        {parseInt(item.effect_amount) > 0 && (
                                                                            <div>Thành tiền làm mới: <span className="text-bold">{utils.formatNumber(item.effect_amount,0,".","đ")}</span></div>
                                                                        )}
                                                                    ...
                                                                    {parseInt(this.props.sales_order.status) !== Constant.STATUS_DELETED &&(
                                                                        <div className="cell-custom">
                                                                            <div className="text-underline pointer">
                                                                                <span className="text-bold text-primary" onClick={()=>{this.btnEdit(item,detailService?.display_name_frontend || detailService?.display_name_contract)}}>Chỉnh sửa</span>
                                                                            </div>
                                                                            <div className="text-underline pointer">
                                                                                <span className="text-bold text-danger" onClick={()=>{this.btnDelete(item,detailService?.display_name_frontend || detailService?.display_name_contract)}}>Xóa</span>
                                                                            </div>
                                                                        </div>
                                                                    )}
                                                                </td>
...
export default connect(mapStateToProps,mapDispatchToProps)(JobBasicPackage);
