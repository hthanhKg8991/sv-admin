...
import PopupEmployerPackage from '../Popup/PopupEmployerPackage';
...
class EmployerPackage extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: true,
    ...
    _btnBuy(){
        this.props.uiAction.createPopup(PopupEmployerPackage, "Mua Điểm Dịch Vụ", {sales_order: this.props.sales_order});
    }
    ...
    _btnEdit(object){
        this.props.uiAction.createPopup(PopupEmployerPackage, "Chỉnh Sửa Mua Điểm Dịch Vụ", {
    ...
    _btnDelete(object){
        this.props.uiAction.SmartMessageBox({
            title: "Bạn có chắc muốn xóa gói điểm dịch vụ ?",
            content: "",
            buttons: ['No','Yes']
        }, (ButtonPressed) => {
            ...
            <div className="col-result-full crm-section">
                <div className="box-card box-full">
                    <div className="box-card-title pointer box-package" onClick={this.showHide}>
                        <span className="title left">Điểm dịch vụ ({data_list.length})</span>
                        ...
                        <div>
                            {this.state.loading ? (
                                ...
                                <div className="card-body">
                                    {parseInt(sales_order.status) !== Constant.STATUS_DELETED && !data_list.length && (
                                        <div className="left">
                                                            ...
                                                            <tr key={key} className={classnames("el-table-row", (key % 2 !== 0 ? "tr-background" : ""))}>
                                                                <td>
                                                                    <div className="cell-custom">
                                                                        ...
                                                                        <div>Thời gian hiệu lực: <span className="text-bold">{item.service_items_info.week_quantity} tuần ({moment.unix(item.start_date).format("DD/MM/YYYY")} - {moment.unix(item.end_date).format("DD/MM/YYYY")})</span></div>
                                                                    </div>
                                                                </td>
                                                                <td>
                                                                    <div className="cell-custom">
...
export default connect(mapStateToProps,mapDispatchToProps)(EmployerPackage);
