...
import PopupJobPackage from '../Popup/PopupJobPackage';
...
class JobPackage extends Component {
    ...
    _btnBuy(){
        this.props.uiAction.createPopup(PopupJobPackage, "Mua Tin Tính Phí", {sales_order: this.props.sales_order});
    }
    _btnEdit(object){
        this.props.uiAction.createPopup(PopupJobPackage, "Chỉnh Sửa Mua Tin Tính Phí", {
    ...
    componentWillReceiveProps(newProps) {
        ...
        if (newProps.refresh['JobPackage']){
            let delay = newProps.refresh['JobPackage'].delay ? newProps.refresh['JobPackage'].delay : 0;
            this.refreshList(delay);
            this.props.uiAction.deleteRefreshList('JobPackage');
        }
    }
            ...
            <div className="col-result-full crm-section">
                <div className="box-card box-full">
                    <div className="box-card-title pointer box-package" onClick={this.showHide}>
                        <span className="title left">Tin tính phí và hiệu ứng ({data_list.length})</span>
                                    ...
                                    {parseInt(sales_order.status) !== Constant.STATUS_DELETED &&(
                                        <div className="left">
                                            <button type="button" className="el-button el-button-primary el-button-small" onClick={this.btnBuy}>
                                                <span>Mua tin tính phí <i className="glyphicon glyphicon-plus"/></span>
                                            </button>
                                        </div>
                                    )}
                                    <div className="right">
                                                    ...
                                                    {data_list.map((item, key)=> {
                                                        return (
                                                            <tr key={key} className={classnames("el-table-row", (key % 2 !== 0 ? "tr-background" : ""))}>
                                                                ...
                                                                <td>
                                                                    <div className="cell-custom">
                                                                        <div>Thành tiền tin phí: <span className="text-bold">{utils.formatNumber(item.original_amount,0,".","đ")}</span></div>
                                                                        {parseInt(item.effect_amount) > 0 && (
                                                                            <div>Thành tiền hiệu ứng: <span className="text-bold">{utils.formatNumber(item.effect_amount,0,".","đ")}</span></div>
                                                                        )}
...
export default connect(mapStateToProps,mapDispatchToProps)(JobPackage);
