...
import PlanTargetInWeekTeam from './PlanTargetInWeekTeam'
...
class index extends Component {
    ...
    _refreshList(delay = 0, update = false){
        ...
        params['week'] = moment().isoWeek();
        if (!update){
            this.setState({loading: true});
            this.setState({itemActive: {}});
        }
        this.props.apiAction.requestApi(apiFn.fnGet, config.apiPlanDomain, ConstantURL.API_URL_PLAN_WEEK_LIST, params, delay);
    }
    componentWillMount(){

    }
    componentWillReceiveProps(newProps) {
        if (newProps.api[ConstantURL.API_URL_PLAN_WEEK_LIST]){
            let response = newProps.api[ConstantURL.API_URL_PLAN_WEEK_LIST];
            if (response.code === Constant.CODE_SUCCESS) {
                this.setState({data_list: response.data});
            }
            this.setState({loading: false});
            this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_PLAN_WEEK_LIST);
        }
        if (newProps.refresh['PlanTargetInWeek']){
            let refresh = newProps.refresh['PlanTargetInWeek'];
            let delay = refresh.delay ? refresh.delay : 0;
            this.refreshList(delay, refresh.update);
            this.props.uiAction.deleteRefreshList('PlanTargetInWeek');
        }
    }
    ...
    render () {
        let firstday = moment().isoWeekday(1).format("DD/MM/YYYY");
        let lastday = moment().isoWeekday(7).format("DD/MM/YYYY");
                ...
                <div>
                    <div className="sub-title-form crm-section inline-block">
                        <div className={this.state.show_detail ? "active pointer" : "pointer"} onClick={this.showDetail}>
                            Mục tiêu bán hàng trong tuần ({firstday} - {lastday}) <i aria-hidden="true" className="v-icon material-icons v-icon-append" style={{lineHeight:"15px"}}>arrow_drop_down</i>
                        </div>
                    </div>
                                ...
                                <DragScroll allowSelect={false}>
                                    <table className="table-default">
                                        ...
                                        {data_list.map((item, key)=> {
                                            return(
                                                <PlanTargetInWeekTeam key={key} item={item} itemActive={itemActive} activeItem={this.activeItem}/>
                                            )
                                        })}
                                        </tbody>
                                    </table>
