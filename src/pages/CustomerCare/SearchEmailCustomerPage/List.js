...
import {employerEmailCustomer} from "api/employer";
...
class List extends Component {
    ...
    async _onChangeFileImport(event) {
        ...
        if (Constant.EXTENSION_FILE_IMPORT.includes(ext)) {
            ...
            const resImport = await employerEmailCustomer(body);
            if (resImport) {
        ...
        return (
            <Default
                ...
                <ul>
                    ...
                    <li>Cột 1 là cột Mã số thuế/CMND cần tra cứu (Bắt đầu lấy giá trị từ ô A1)</li>
                    <li>Cột mã số thuế/CMND cần tra cứu theo format: <span className="text-red">MST-[mã số thuế] (VD: MST-0982345444)</span> </li>
                </ul>
            </Default>
        )
    }
