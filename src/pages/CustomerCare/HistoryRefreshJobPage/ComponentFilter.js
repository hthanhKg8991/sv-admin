...
class ComponentFilter extends Component {
    ...
    render () {
        ...
        let service_list_refresh = sys.effect?.items.filter(_ => _.code.indexOf("refresh") > 0);
        return (
            <FilterLeft idKey={idKey} query={query} menuCode={menuCode}>
                <SearchField type="input" label="ID, Tiêu đề Tin" name="job_q" timeOut={1000}/>
                <SearchField type="input" label="ID, Tên NTD" name="employer_q" timeOut={1000}/>
                <SearchField type="dropboxmulti" label="CSKH" name="assigned_staff_id" key_value="id" key_title="login_name" data={staff_list}/>
                <SearchField key_value="code" key_title="name" type="dropbox" label="Loại phí" name="effect_service_code" data={service_list_refresh}/>
            </FilterLeft>
        )
    }
}
