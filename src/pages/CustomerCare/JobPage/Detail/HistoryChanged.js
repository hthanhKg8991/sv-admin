...
import {getListRevision} from "api/job";
...
import Detail from "pages/CustomerCare/JobPage/Detail/HistoryChanged/Detail";
...
class HistoryChanged extends React.Component {
        ...
        this.state = {
            ...
            columns: [
                ...
                },
                {
                    ...
                    width: 100,
                    cell: row => (
                        <SpanCommon idKey={Constant.COMMON_DATA_KEY_job_status} value={row.revision_status}/>
                    )
                },
                {
                    title: "Người duyệt",
                        ...
                        <React.Fragment>
                            {_.get(row, 'revision_reason', null) && _.get(row, 'revision_reason').map(reason => (
                                <React.Fragment key={reason}>
                                    - <SpanCommon idKey={Constant.COMMON_DATA_KEY_job_rejected_reason}
                                                  value={reason}
                                                  notStyle/><br/>
                                </React.Fragment>
                            ))}
    ...
    _expandRow(row) {
        ...
        return (<Detail detailId={row.id} jobId={id} history={history}/>);
    }
    ...
    render() {
        ...
        return (
            <Default title={'Lịch sử thay đổi Tin'}
                     titleActions={(
