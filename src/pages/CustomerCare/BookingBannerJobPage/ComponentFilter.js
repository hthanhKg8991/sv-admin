...
class ComponentFilter extends Component {
    ...
    _getBoxList(changeUrl = false){
        this.props.apiAction.requestApi(apiFn.fnGet, config.apiBookingDomain, ConstantURL.API_URL_GET_LIST_BOX_BANNER, {changeUrl: changeUrl});
    }
    _onSearch(params){
        ...
        this.props.uiAction.refreshList('BookingBannerJobPage');
    }
    componentWillMount(){
        this.getBoxList(false);
    }
    componentWillReceiveProps(newProps) {
        if (newProps.api[ConstantURL.API_URL_GET_LIST_BOX_BANNER] && Object.entries(newProps.sys.common.items).length !== 0) {
            let response = newProps.api[ConstantURL.API_URL_GET_LIST_BOX_BANNER];
            let changeUrl = response.info?.args?.changeUrl;
            if (response.code === Constant.CODE_SUCCESS) {
                let params = queryString.parse(window.location.search);
                if (response.data && response.data[0] && (!params.booking_box_id || changeUrl)) {
                    params.booking_box_id = response.data[0].id;
                }
                ...
                if (area_list && area_list[0] && (!params.displayed_area || changeUrl)){
                    params.displayed_area = area_list[0].value;
                }
    ...
    render () {
        ...
        const isField = jobField && jobField.length > 0;

        return (
            <BoxSearch showQtty={4} onChange={this.onSearch}>
                {/*displayed_area*/}
                <SearchField type="dropbox" label="Khu vực hiển thị" name="displayed_area" data={area_list} noDelete/>
                {/*booking_box_id*/}
                <SearchField type="dropbox" label="Gói dịch vụ" name="booking_box_id" data={box_list} key_value="id" key_title="name" noDelete/>
                {/*job_field_id*/}
                {isField ? <SearchField type="dropbox" label="Ngành nghề" name="job_field_id" data={jobField} key_value="id" key_title="name"/>: <></>}
            </BoxSearch>
        )
    }
}
