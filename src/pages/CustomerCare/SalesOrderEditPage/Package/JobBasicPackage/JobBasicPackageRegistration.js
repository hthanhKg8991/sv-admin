...
import PopupJobBasicRegistration from "../../Popup/PopupJobBasicRegistration";
...
class JobBasicPackageRegistration extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
    ...
    _btnEdit(object) {
        this.props.uiAction.createPopup(PopupJobBasicRegistration, "Chỉnh Sửa Đăng Ký Tin Cơ Bản", {
    ...
    _btnApproveRegistration(id, titleParam, continueParam,paramPass , index = 0) {
        const {currentBranch} = this.props.branch;
        if(currentBranch?.channel_code === "vl24h"){
            this.props.uiAction.SmartMessageBoxV2({
                title: titleParam || "Bạn có chắc muốn duyệt gói cơ bản?",
                content: "",
                uniqueKey: "accountRegisJobBasicApprove" + id + index,
                buttons: [
                    {
                        text: "Yes",
                        className: "el-button-success",
                        callback: async () => {
                            this.props.uiAction.showLoading()
                            const paramsApprove = paramPass || { registration_job_box_id: id }
                            if(continueParam){
                                paramsApprove[continueParam] = true
                            }
                            const response = await accountRegisJobBasicApprove(paramsApprove);
                            this.props.uiAction.hideLoading()
                            if (response?.code === Constant.CODE_RES_CONFIRM) {
                                this._btnApproveRegistration(id, response?.msg, "allowed_continue",paramsApprove ,index + 1)
                            }else if(response?.code === Constant.CODE_RES_CONFIRM_UPDATE_END_DATE){
                                this._btnApproveRegistration(id, response?.msg, "allowed_update_end_date",paramsApprove ,index + 1)
                            }else if (response?.code === Constant.CODE_RES_ALERT) {
                                this._btnApproveRegistration(id, response?.msg, "run_continue",paramsApprove ,index + 1)
                                    ...
                                    this.props.uiAction.SmartMessageBox({
                                        ...
                                        if (ButtonPressed === "Yes") {
                                            ...
                                            if (res_cont?.code === Constant.CODE_SUCCESS) {
                                                ...
                                                this.props.uiAction.putToastError(res_cont.msg);
    ...
    async componentWillReceiveProps(newProps) {
        ...
        if (newProps.refresh['JobBasicPackageRegistration']) {
            let refresh = newProps.refresh['JobBasicPackageRegistration'];
            if (refresh.sales_order_items_id === this.props.sales_order_item.id) {
                publish(".refresh", {}, this.props.idKey);
                this.props.uiAction.deleteRefreshList('JobBasicPackageRegistration');
            }
        }
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(JobBasicPackageRegistration);
