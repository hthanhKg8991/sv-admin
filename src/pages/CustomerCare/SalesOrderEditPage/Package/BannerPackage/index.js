...
import PopupBannerPackage from 'pages/CustomerCare/SalesOrderEditPage/Package/Popup/PopupBannerPackage';
...
import BannerSubItem from "pages/CustomerCare/SalesOrderEditPage/SubItem/BannerSubItem";
import SpanCommon from "components/Common/Ui/SpanCommon";

const idKey = Constant.IDKEY_BANNER_PACKAGE;
...
class index extends Component {
    ...
    _btnBuy(){
        this.props.uiAction.createPopup(PopupBannerPackage, "Mua Quảng bá thương hiệu", {sales_order: this.props.sales_order});
    }

    _btnGift(){
        this.props.uiAction.createPopup(PopupBannerPackage, "Mua Quảng bá thương hiệu (Tặng)", {
            sales_order: this.props.sales_order,
            type_campaign: Constant.SALES_ORDER_ITEM_TYPE_CAMPAIGN.gift
        });
    }
    _btnDuplicate(object){
        this.props.uiAction.createPopup(PopupBannerPackage, "Mua Quảng bá thương hiệu", {
            sales_order: this.props.sales_order,
    ...
    _btnEdit(object){
        this.props.uiAction.createPopup(PopupBannerPackage, "Chỉnh Sửa Mua Quảng bá thương hiệu", {
            sales_order: this.props.sales_order,
            object: object
        });
    }
    _btnDelete(object){
        ...
        this.props.uiAction.SmartMessageBox({
            title: "Bạn có chắc muốn xóa gói banner ?",
            content: "",
            buttons: ['No','Yes']
        }, (ButtonPressed) => {
    ...
    componentWillReceiveProps(newProps) {
        ...
        if (newProps.refresh['BannerPackage']){
            let delay = newProps.refresh['BannerPackage'].delay ? newProps.refresh['BannerPackage'].delay : 0;
            this.refreshList(delay);
            this.props.uiAction.deleteRefreshList('BannerPackage');
        }
    }
    ...
    render () {
        ...
        let {sales_order, is_action = true} = this.props;
                                            ...
                                            <div className="left">
                                                <CanRender actionCode={ROLES.customer_care_sales_order_registration_service_manage}>
                                                    ...
                                                    <button type="button"
                                                            ...
                                                            onClick={this.btnBuy}>
                                                    <span>Mua Quảng bá thương hiệu <i
                                                    ...
                                                    <span>Mua Quảng bá thương hiệu (Tặng) <i
                                                        className="glyphicon glyphicon-plus"/></span>
                                                    </button>
                                                </CanRender>
                                            </div>
                                        ...
                                        <div className="body-table el-table">
                                            <TableComponent className="table-custom">
                                                <TableHeader tableType="TableHeader" width={400}>
                                                    Thông tin mua Quảng bá thương hiệu
                                                </TableHeader>
                                                <TableHeader tableType="TableHeader" width={400}>
                                                    Giá
                                                </TableHeader>
                                                                ...
                                                                <tr className={classnames("el-table-row pointer")}>
                                                                    <td onClick={()=>{this.activeItem(item.id)}}>
                                                                        <div className="cell-custom">
                                                                            ...
                                                                            <div>Khu vực: <span className="text-bold">{area[item.service_items_info.displayed_area]}</span></div>
                                                                            {item.service_items_info.displayed_method &&
                                                                                ...
                                                                                if (parseInt(price) === displayed_area || displayed_area === Constant.AREA_ALL) {
                                                                                    return (
                                                                                        <div key={key_price} className="mb10-child">
                                                                                            ...
                                                                                            <div>Bảng giá: <span className="text-bold">{item.price_list_info[price]?.title}</span>
                                                                                            </div>
                                                                                            <div>
                                                                    ...
                                                                    <td onClick={()=>{this.activeItem(item.id)}}>
                                                                        <div className="cell-custom">
                                                                            <div>Thành tiền banner: <span className="text-bold">{utils.formatNumber(item.original_amount,0,".","đ")}</span></div>
