...
import PopupEffectPackage from 'pages/CustomerCare/SalesOrderEditPage/Package/Popup/PopupEffectPackage';
import PopupEditEffectPackage from 'pages/CustomerCare/SalesOrderEditPage/Package/Popup/PopupEditEffectPackage'
...
import EffectSubItem from "pages/CustomerCare/SalesOrderEditPage/SubItem/EffectSubItem";
...
class index extends Component {
    ...
    _btnBuy(){
        this.props.uiAction.createPopup(PopupEffectPackage, "Mua Hiệu Ứng", {sales_order: this.props.sales_order});
    }
    _btnGift() {
        this.props.uiAction.createPopup(PopupEffectPackage, "Mua Hiệu Ứng (Tặng)", {
    ...
    _btnEdit(object){
        this.props.uiAction.createPopup(PopupEditEffectPackage, "Chỉnh Sửa Mua Hiệu Ứng", {
            sales_order: this.props.sales_order,
            object: object
        });
    }
    _btnDelete(object){
        ...
        this.props.uiAction.SmartMessageBox({
            title: "Bạn có chắc muốn xóa hiệu ứng ?",
            content: "",
            buttons: ['No','Yes']
        }, (ButtonPressed) => {
    ...
    async _getConfig() {
        ...
        if (res && res?.items?.length > 0) {
            ...
            this.setState({isPriceDefaultEffect:
    ...
    componentDidMount() {
        ...
        this.getConfig();  // Get config của giá gói hiệu ứng (đối với kênh có config: mw)
    }
    ...
    componentWillReceiveProps(newProps) {
        ...
        if (newProps.refresh['EffectPackage']){
            let delay = newProps.refresh['EffectPackage'].delay ? newProps.refresh['EffectPackage'].delay : 0;
            this.refreshList(delay);
            this.props.uiAction.deleteRefreshList('EffectPackage');
        }
    }
    ...
    renderButtons = (item) => {
        const {sales_order, is_action = true, channelCode, branch, is_sub = false, sys, subscription} = this.props;
        ...
        const box_code_list = sys.service.items.filter(c =>
            ...
            && this.state.package_running.includes(c.code) // kiểm tra các gói đang chạy trong bảng giá
        );

        const box_code = box_code_list.filter(c => c.code === item?.service_items_info?.parent_service_code);
        ...
        if ([Constant.CHANNEL_CODE_TVN, Constant.CHANNEL_CODE_VL24H, Constant.CHANNEL_CODE_MW].includes(channelCode)) {
            if (![
                ...
                if (is_sub && !is_basic && [Constant.SUBSCRIPTION_TYPE_PRO_VALUE, Constant.SUBSCRIPTION_TYPE_PLUS_VALUE].includes(subscription?.type_campaign) && channel_code !== Constant.CHANNEL_CODE_TVN){
                    return(
                        <>
                            <div className="text-underline pointer mt10 mb10">
    ...
    render () {
        let {data_list, show_detail, itemActive, isPriceDefaultEffect} = this.state;
            ...
            <div className="col-result-full crm-section">
                <div className="box-card box-full">
                    <div className="box-card-title pointer box-package" onClick={this.showHide}>
                        <span className="title left">Hiệu ứng ({total})</span>
                                            ...
                                            <div className="left">
                                                <CanRender
                                                    ...
                                                    <button type="button"
                                                            ...
                                                            onClick={this.btnBuy}>
                                                        <span>Mua hiệu ứng <i className="glyphicon glyphicon-plus"/></span>
                                                    </button>
                                                    }
                                                    <button type="button"
                                                            className="el-button el-button-primary el-button-small"
                                                            onClick={this.btnGift}>
                                                        <span>Mua hiệu ứng (Tặng) <i className="glyphicon glyphicon-plus"/></span>
                                                    </button>
                                                </CanRender>
                                            </div>
                                        )
                                                                ...
                                                                <tr className={classnames("el-table-row pointer")}>
                                                                    <td onClick={()=>{this.activeItem(item.id)}}>
                                                                        <div className="cell-custom">
                                                                            ...
                                                                            <div>Tên gói dịch vụ: <span className="text-bold">{item.service_items_info.cache_parent_service_name}</span></div>
                                                                            <div>Mã SKU: <span
                                                                            ...
                                                                            <div>Hiệu ứng: <span className="text-bold">{item.cache_service_name}</span></div>
                                                                                    ...
                                                                                    return (
                                                                                        <div key={key_price} className="mb10-child">
                                                                                            ...
                                                                                            {isPriceDefaultEffect ? (
                                                                                                ...
                                                                                                <div>
                                                                                                    Giá niêm yết: <span className="text-bold">{utils.formatNumber(item.service_items_info?.unit_price_percent
                                                                                                        ? item.price_list_info[price]?.based_price * Number(item.service_items_info.unit_price_percent) / 100
                                                                                                        : item.price_list_info[price]?.based_price, 0, ".", "đ")} ({utils.formatNumber(item.price_list_info[price]?.unit_per_based, 0, ".", "ngày")})</span>
                                                                                                </div>
                                                                                            )}
                                                                    ...
                                                                    <td onClick={()=>{this.activeItem(item.id)}}>
                                                                        <div className="cell-custom">
                                                                            <div>Thành tiền hiệu ứng: <span className="text-bold">{utils.formatNumber(item.original_amount,0,".","đ")}</span></div>
