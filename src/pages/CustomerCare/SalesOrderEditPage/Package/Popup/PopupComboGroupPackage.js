...
import {createGroupComboItem, getDetailProductGroupList, getListCombo} from "api/saleOrder";
import {publish} from "utils/event";
import * as Constant from "utils/Constant";
import Dropbox from "components/Common/InputValue/Dropbox";
import Input2 from "components/Common/InputValue/Input2";

class PopupComboGroupPackage extends Component {
    constructor(props) {
        super(props);
        this.state = {
            object: {
    ...
    async _onSave(data) {
        ...
        const res = await createGroupComboItem(object);
        if (res) {
            this.props.uiAction.putToastSuccess("Thao tác thành công!");
            this.props.uiAction.deletePopup();
            publish(".refresh", {}, Constant.IDKEY_COMBO_PACKAGE);
    ...
    async _getListItemsGroup() {
        const res = await getListCombo({ filter_expired: true,status: Constant.STATUS_ACTIVED, per_page: 100});
...
export default connect(mapStateToProps, mapDispatchToProps)(PopupComboGroupPackage);
