...
import PopupEmployerPackageRegistration from "../../Popup/PopupEmployerPackageRegistration";
...
import {accountRegisFilterApprove} from "api/saleOrder"

class EmployerPackageRegistration extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
    ...
    _btnEdit(object){
        this.props.uiAction.createPopup(PopupEmployerPackageRegistration, "Chỉnh Sửa Đăng Ký Điểm Dịch Vụ", {
    ...
    _btnDelete(object){
        this.props.uiAction.SmartMessageBox({
            title: "Bạn có chắc muốn xóa đăng ký gói điểm dịch vụ ?",
            content: "",
            buttons: ['No','Yes']
        }, (ButtonPressed) => {
            if (ButtonPressed === "Yes") {
                this.props.uiAction.showLoading();
                this.props.apiAction.requestApi(apiFn.fnPost, config.apiSalesOrderDomain, ConstantURL.API_URL_POST_REGIS_FILTER_RESUME_DELETE, {
                    id: object.id,
    ...
    _btnRegisCancel(object){
        this.props.uiAction.createPopup(PopupRegisCancel, "Đăng Ký Hạ Dịch Vụ", {
            ...
            refresh_page: 'EmployerPackageRegistration',
            url_reject: ConstantURL.API_URL_POST_SALES_ORDER_REGIS_CANCEL_FILTER_RESUME,
            idKey: Constant.IDKEY_EMPLOYER_PACKAGE
        });
    }
    ...
    _btnApproveRegistration(id){
        this.props.uiAction.SmartMessageBox({
            ...
            buttons: ['No','Yes']
				...
				if (ButtonPressed === "Yes") {
					this.props.uiAction.showLoading();
					const params = {registration_filter_resume_id: id};
					const res = await accountRegisFilterApprove(params);
					if (res?.code === Constant.CODE_SUCCESS) {
						...
						if (confirm) {
							const resConfirm = await accountRegisFilterApprove({...params, allowed_update_end_date: true});
    ...
    _btnApproveCancel(item){
        this.props.uiAction.SmartMessageBox({
            title: "Bạn có chắc muốn duyệt hạ gói lọc ?",
            content: "",
            buttons: ['No','Yes']
        }, (ButtonPressed) => {
            if (ButtonPressed === "Yes") {
    ...
    componentWillReceiveProps(newProps) {
        if (newProps.api[ConstantURL.API_URL_POST_REGIS_FILTER_RESUME_DELETE]){
            let response = newProps.api[ConstantURL.API_URL_POST_REGIS_FILTER_RESUME_DELETE];
            ...
            if (id === this.props.sales_order_item.id) {
                ...
                this.props.uiAction.hideLoading();
                this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_REGIS_FILTER_RESUME_DELETE);
            }
        }
        if (newProps.refresh['EmployerPackageRegistration']){
            let refresh = newProps.refresh['EmployerPackageRegistration'];
            if (refresh.sales_order_items_id === this.props.sales_order_item.id) {
                publish(".refresh", {}, this.props.idKey);
                this.props.uiAction.deleteRefreshList('EmployerPackageRegistration');
            }
        }
...
export default connect(mapStateToProps,mapDispatchToProps)(EmployerPackageRegistration);
