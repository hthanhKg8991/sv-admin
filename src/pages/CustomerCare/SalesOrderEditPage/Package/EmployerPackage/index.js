...
import EmployerSubItem from "pages/CustomerCare/SalesOrderEditPage/SubItem/EmployerSubItem";
...
class index extends Component {
    ...
    _btnBuy(name){
        this.props.uiAction.createPopup(PopupEmployerPackage, "Mua " + name, {sales_order: this.props.sales_order});
    }
    _btnGift(name) {
        this.props.uiAction.createPopup(PopupEmployerPackage, `Mua ${name} (Tặng)`, {
    ...
    _btnEdit(object,name){
        this.props.uiAction.createPopup(PopupEmployerPackage, "Chỉnh Sửa Mua " + name, {
            sales_order: this.props.sales_order,
            object: object
        });
    }
    ...
    componentWillReceiveProps(newProps) {
        ...
        if (newProps.refresh['EmployerPackage']){
            let delay = newProps.refresh['EmployerPackage'].delay ? newProps.refresh['EmployerPackage'].delay : 0;
            this.refreshList(delay);
            this.props.uiAction.deleteRefreshList('EmployerPackage');
        }
    }
    ...
    render () {
        ...
        let {sales_order, is_action = true, isOutdated = false, sys} = this.props;
            ...
            <div className="col-result-full crm-section">
                <div className="box-card box-full">
                    <div className="box-card-title pointer box-package" onClick={this.showHide}>
                        <span className="title left">{detailService?.display_name_frontend || detailService?.display_name_contract} ({total})</span>
                                        ...
                                        is_action && (
                                            <div className="left">
                                                <CanRender actionCode={ROLES.customer_care_sales_order_registration_service_manage}>
                                                    ...
                                                    <button type="button"
                                                            className="el-button el-button-primary el-button-small"
                                                            onClick={()=>this.btnGift(detailService?.display_name_frontend || detailService?.display_name_contract)}>
                                                        <span>Mua {detailService?.display_name_frontend || detailService?.display_name_contract} (Tặng) <i className="glyphicon glyphicon-plus"/></span>
                                                    </button>
                                                </CanRender>
                                            </div>
                                        )
                                                                ...
                                                                <tr className={classnames("el-table-row pointer")}>
                                                                    <td onClick={()=>{this.activeItem(item.id)}}>
                                                                        <div className="cell-custom">
                                                                            ...
                                                                            <div>Điểm mua: <span className="text-bold">{utils.formatNumber(item?.service_items_info?.quantity,0,".","điểm")}</span></div>
                                                                            <div>Thời gian hiệu lực: <span className="text-bold">
                                                                                {item.service_items_info?.week_quantity} tuần {item?.service_items_info?.day_quantity > 0 &&
                                                                                `${item?.service_items_info?.day_quantity} ngày`
                                                                            }
                                                                                {item?.start_date &&
                                                                    ...
                                                                    <td>
                                                                        <div className="cell-custom">
                                                                            {this.renderButtons(item,detailService?.display_name_frontend || detailService?.display_name_contract)}
                                                                            {/*<div className="text-underline pointer">*/}
