...
class PopupEmployerPackageRegistration extends Component {
    constructor(props) {
        super(props);
        this.state = {
            object: {},
    ...
    _onSave(data, required){
        ...
        if (!object.id){
            this.props.apiAction.requestApi(apiFn.fnPost, config.apiSalesOrderDomain, ConstantURL.API_URL_POST_REGIS_FILTER_RESUME_CREATE, object);
        }else{
            this.props.apiAction.requestApi(apiFn.fnPost, config.apiSalesOrderDomain, ConstantURL.API_URL_POST_REGIS_FILTER_RESUME_EDIT, object);
        }
    }
    ...
    _getDetail(id){
        ...
        this.setState({loading: true});
        this.props.apiAction.requestApi(apiFn.fnGet, config.apiSalesOrderDomain, ConstantURL.API_URL_GET_REGIS_FILTER_RESUME_DETAIL, args);
    }
    _getSalesOrderItem(){
        let args = {
    ...
    componentWillReceiveProps(newProps) {
        ...
        if (newProps.api[ConstantURL.API_URL_GET_REGIS_FILTER_RESUME_DETAIL]){
            let response = newProps.api[ConstantURL.API_URL_GET_REGIS_FILTER_RESUME_DETAIL];
            if (response.code === Constant.CODE_SUCCESS) {
                let object = {};
            ...
            this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_GET_REGIS_FILTER_RESUME_DETAIL);
        }
        if (newProps.api[ConstantURL.API_URL_POST_REGIS_FILTER_RESUME_CREATE]){
            let response = newProps.api[ConstantURL.API_URL_POST_REGIS_FILTER_RESUME_CREATE];
            if (response.code === Constant.CODE_SUCCESS) {
            ...
            this.props.uiAction.hideLoading();
            this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_REGIS_FILTER_RESUME_CREATE);
        }
        if (newProps.api[ConstantURL.API_URL_POST_REGIS_FILTER_RESUME_EDIT]){
            let response = newProps.api[ConstantURL.API_URL_POST_REGIS_FILTER_RESUME_EDIT];
            if (response.code === Constant.CODE_SUCCESS) {
                this.props.uiAction.putToastSuccess("Thao tác thành công!");
                this.props.uiAction.deletePopup();
                publish(".refresh", {}, Constant.IDKEY_EMPLOYER_PACKAGE);
            ...
            this.props.uiAction.hideSmartMessageBox();
            this.props.uiAction.hideLoading();
            this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_REGIS_FILTER_RESUME_EDIT);
        }
    }
...
export default connect(mapStateToProps,mapDispatchToProps)(PopupEmployerPackageRegistration);
