...
import JobSubItemRow from "pages/CustomerCare/SalesOrderEditPage/SubItem/JobSubItem/Row";
import PopupJobPackageRegistration from "pages/CustomerCare/SalesOrderEditPage/Popup/PopupJobPackageRegistration";
...
class JobSubItem extends Component {
    ...
    _btnAdd(){
        this.props.uiAction.createPopup(PopupJobPackageRegistration, "Đăng Ký Tin Tính Phí", {
    ...
    render() {
        ...
        const {isOutdated = false, isChangeArea} = this.props;
        if(!items) {
            return null;
        }
        return (
                    ...
                    (
                        <CanRender actionCode={ROLES.customer_care_sales_order_registration_service_manage}>
                            <button type="button" className="el-button el-button-primary el-button-small"
                                    onClick={this.btnAdd}>
                                <span>Đăng ký tin tính phí <i className="glyphicon glyphicon-plus"/></span>
                            </button>
                        </CanRender>
                    )}
                </div>
                            ...
                            <div key={idx.toString()} className="mb20">
                                <div className="box-card box-full">
                                    <div className="box-card-title pointer box-package d-flex">
                                        ...
                                        {_?.status === Constant.SALE_ORDER_REVERSE_PROCCESS && <span className="title ml30">Số ngày bảo lưu thực tế: {remainingExpiredDay ? `${remainingExpiredDay} ngày` : ''}</span>}
                                    </div>
                                    <div className="collapse in">
                                        <div className="card-body">
                                            <JobSubItemRow isChangeArea={isChangeArea} {...this.props} sub_item={_} data_list={_?.registration_info}/>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(JobSubItem);
