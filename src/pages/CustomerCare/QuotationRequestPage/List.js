...
import {getListQuotationRequest} from "api/saleOrder";
import {bindActionCreators} from 'redux';
import {connect} from "react-redux";
import SpanCommon from "components/Common/Ui/SpanCommon";
import CanRender from "components/Common/Ui/CanRender";
import ROLES from "utils/ConstantActionCode";
import ChangeStatusPopup from "pages/CustomerCare/QuotationRequestPage/Popup/ChangeStatus";

const idKey = "QuotationRequest";
...
class List extends Component {
        ...
        this.state = {
            columns: [
                ...
                },
                {
                    title: "Lọai",
                    width: 80,
                    cell: row => <SpanCommon idKey={Constant.COMMON_DATA_KEY_quotation_request_type}
                                             value={row?.type}/>
                },
                {
                    title: "CSKH",
    ...
    _onDetail(row) {
        ...
        const {id} = row;
        history.push({
            pathname: Constant.BASE_URL_QUOTATION_REQUEST,
    ...
    render() {
        ...
        return (
            <Default
                ...
                title="Danh Sách NTD Đăng Ký Từ Bảng Giá"
                titleActions={(
                ...
                <Gird idKey={idKey}
                      fetchApi={getListQuotationRequest}
                      query={query}
                      columns={columns}
                      defaultQuery={{}}
                      history={history}
