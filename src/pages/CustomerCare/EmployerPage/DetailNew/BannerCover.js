...
import {getBannerCover, updateBannerCover} from "api/employer";
...
import LibraryBannerCoverRow from "pages/CustomerCare/EmployerPage/DetailNew/LibraryBannerCover/LibraryBannerCoverRow";
...
class BannerCover extends React.Component {
    ...
    async asyncData() {
        ...
        const res = await getBannerCover(_.get(employer, 'id'));
        if (res) {
            let dataList = [];
    ...
    async asyncSubmit() {
        ...
        const res = await updateBannerCover({
            ...
            <React.Fragment>
                ...
                <div className={"row mt20"}>
                    {imageList?.map((item, key) => (
                        <LibraryBannerCoverRow className="col-xs-12 col-sm-12 mb50 clearfix" key={key.toString()} keygroup={key}
                                         item={item} channel_code={channel_code} onChange={this.onChange}/>
                    ))}
...
export default connect(null, mapDispatchToProps)(BannerCover);
