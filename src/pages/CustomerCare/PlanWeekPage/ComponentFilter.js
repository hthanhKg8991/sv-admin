...
class ComponentFilter extends Component {
    ...
    _onSearch(params){
        ...
        this.props.uiAction.refreshList('PlanWeekPage');
    }
    _getCustomerCare(){
    ...
    componentWillMount(){
        ...
        let week = String(moment().week()).padStart(2, '0');
        params['week_year'] = params['week_year'] ? params['week_year'] : week+"/"+moment().year();
        this.props.history.push(window.location.pathname + '?' + queryString.stringify(params));
        this.setState({params: params});
    ...
    render () {
        ...
        let weeks = utils.getListWeekInYear();
        return (
            <BoxSearch showQtty={4} onChange={this.onSearch}>
                ...
                <SearchField type="dropbox" label="Tuần " name="week_year" data={weeks} valueDefault={this.state.week_year} noDelete/>
            </BoxSearch>
        )
    }
}
