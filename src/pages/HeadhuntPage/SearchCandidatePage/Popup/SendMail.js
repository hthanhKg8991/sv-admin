...
class PopupChangeSendMail extends Component {
    ...
    async _onSave(data, required) {
        const {uiAction,clearCheckSendMail, check_send_mail} = this.props;
        ...
        uiAction.SmartMessageBox({
            ...
            if (ButtonPressed === "Yes") {
                ...
                const res = await createMultipleListContact({...object, data : check_send_mail.filter(v => v.token_email === "verified")});
                if (res) {
                    clearCheckSendMail();
