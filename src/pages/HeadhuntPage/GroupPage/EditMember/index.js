...
import {createHeadhuntGroupMember, updateHeadhuntGroupMember} from "api/headhunt";
...
class EditMember extends React.Component {
    ...
    async submitData(data, setErrors) {
        const {actions, object, group_id, idKey} = this.props;
        let res;
        data.group_id = group_id;
        if (object?.id > 0) {
            data.id = object.id;
            res = await updateHeadhuntGroupMember(data);
        } else {
            res = await createHeadhuntGroupMember(data);
        }
        if (res) {
    ...
    render() {
        ...
        const validationSchema = Yup.object().shape({
            ...
            role: Yup.string().required(Constant.MSG_REQUIRED),
        });
