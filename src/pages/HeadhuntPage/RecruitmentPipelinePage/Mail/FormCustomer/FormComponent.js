...
import {getListFullTemplateMail} from "api/headhunt";
...
class FormComponent extends React.Component {
    ...
    async _getTemplate () {
        const res = await getListFullTemplateMail({status: Constant.STATUS_ACTIVED});
        if (res){
            const template = res.map(v => ({...v,value: v.id, label: v.name}));
     ...
     _onChangeTemplate (value) {
        ...
        if (Number(value) > 0){
            ...
            if (selected){
                ...
                setFieldValue('content', selected.content).then(()=>{this.setState({showCkEditor: true})});
    ...
    render() {
        const {template, showCkEditor, cc} = this.state;
        return (
            <React.Fragment>
                <div className="row">
                ...
                <div className={"row"}>
                    <div className="col-md-12 mb10">
                        <MyUpload name={"attached_file_url"} label={"CV ứng viên"}
