...
import { updateHeadhuntApplicantInfo} from "api/headhunt";
...
class Edit extends React.Component {
    ...
    async submitData(data, setErrors) {
        ...
        if (object?.id > 0) {
            data.id = object.id;
            res = await updateHeadhuntApplicantInfo(data);
        }
        if (res) {
    ...
    render() {
        ...
        const validationSchema = Yup.object().shape({
            email: Yup.string().required(Constant.MSG_REQUIRED),
            phone: Yup.string().required(Constant.MSG_REQUIRED),
