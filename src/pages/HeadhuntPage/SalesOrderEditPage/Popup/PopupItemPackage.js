...
import {
    createSalesOrderHeadhuntItem, getDetailHeadhuntSalesOrderItem,
    getListFullSkuHeadhunt,
...
class PopupItemPackage extends Component {
    constructor(props) {
        ...
        this.state = {
            object: {},
            object_required: [ 'sku_code', 'quantity', 'sku_price' ],
            object_error: {},
            name_focus: "",
            list_sku: [],
        };
    ...
    async _onSave(data, object_required) {
        ...
        let res;
        if (id > 0 ){
            res = await updateSalesOrderHeadhuntItem({...params, id});
        }else {
            res = await createSalesOrderHeadhuntItem(params);
        }
        if (res) {
    ...
    async _getListSku(params = {}) {
        const res = await getListFullSkuHeadhunt({...params,status: Constant.SKU_STATUS_ACTIVE});
        if (res) {
    ...
    async _getDetail() {
        ...
        const res = await getDetailHeadhuntSalesOrderItem({
            id: this.props.id
        });
        if (res) {
                ...
                <div className="dialog-popup-body">
                    <div className="popupContainer">
                        <div className="form-container">
                            ...
                            <div className=" row">
                                <div className="col-xs-6 mb10">
                                    <Input2 type="text" name="sku_price" label="Đơn giá (Đã bao gồm VAT)" isNumber required={object_required.includes('sku_price')}
                                            error={object_error.sku_price} value={object.sku_price} nameFocus={name_focus}
                                            onChange={this.onChange}
                                    />
                                </div>
                            </div>
