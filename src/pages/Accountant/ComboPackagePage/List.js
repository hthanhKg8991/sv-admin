...
import {deleteCombo, getListCombo, toggleActiveCombo} from "api/saleOrder";
...
import ComponentFilter from "pages/Accountant/ComboPackagePage/ComponentFilter";

const idKey = "ComboPackagePageList";
...
class List extends Component {
			...
			columns: [
				...
				},
				{
					...
					cell: row => (
                        ...
                        <>
                        	<CanRender actionCode={ROLES.accountant_combo_package_update}>
                        		<span className="text-link text-blue font-bold" onClick={() => this.onEdit(row?.id)}>Chỉnh sửa</span>
                        	</CanRender> <br/>
                        	{
                        		row?.status === Constant.STATUS_ACTIVED ?
                        			<CanRender actionCode={ROLES.accountant_combo_package_toggle_active}>
                        	...
                        	{
                        		row?.status !== Constant.STATUS_ACTIVED &&
                                <CanRender actionCode={ROLES.accountant_combo_package_delete}>
	...
	_onClickAdd() {
		...
		history.push({
			pathname: Constant.BASE_URL_COMBO_PACKAGE_PAGE,
	...
	async _onChangeStatus(id) {
		const {actions} = this.props;
		this.setState({loading: true});
		const res = await toggleActiveCombo({
			id
		});
		if (res) {
			...
			<Default
				...
				buttons={(
					<div className="left btnCreateNTD">
						<CanRender actionCode={ROLES.accountant_combo_package_create}>
