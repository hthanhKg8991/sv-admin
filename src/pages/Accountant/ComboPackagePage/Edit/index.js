...
import {createCombo, getDetailCombo, updateCombo} from "api/saleOrder";
...
class Edit extends React.Component {
	constructor(props) {
		...
		let defaultItem = {
			...
			conditions: [
				{
					left:Constant.DEFAULT_VALUE_SUBSCRIPTION_CONDITION,
					operation:"",
					right:"",
				}
			]
	...
	async submitData(data, setErrors) {
		...
		if (id > 0) {
			data.id = id;
			res = await updateCombo(data);
		} else {
			res = await createCombo(data);
		}
		if (res) {
			this.setState({loading: false});
	...
	async asyncData() {
		...
		if (id > 0) {
			const res = await getDetailCombo({id});
			if (res) {
