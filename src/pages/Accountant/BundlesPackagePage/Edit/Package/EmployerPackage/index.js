...
import PopupEmployerPackage from "pages/Accountant/BundlesPackagePage/Edit/Package/Popup/PopupEmployerPackage";
...
class index extends React.Component {
			...
			columns: [
				{
					...
					cell: item => <div className="cell-custom mt5 mb5">
						...
						<div>Điểm mua: <span
							className="text-bold">{utils.formatNumber(item?.quantity, 0, ".", "điểm")}</span>
						</div>
				...
				},
				{
					...
					cell: item => <div className="cell-custom">
						<div className="text-underline pointer">
							<span className="text-bold text-info" onClick={() => this.btnUpdate(item,"Điểm dịch vụ")}>Thay đổi</span>
						</div>
						<br/>
						<div className="text-underline pointer">
							<span className="text-bold text-danger" onClick={() => this.btnDelete(item,"Điểm dịch vụ")}>Xóa</span>
						</div>
					</div>
				}
			]
	...
	render() {
		let {data_list, show_detail, columns} = this.state;
		let { sys } = this.props;
