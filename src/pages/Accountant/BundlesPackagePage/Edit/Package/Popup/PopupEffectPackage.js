...
import {createBundleItem, getListPriceRunning} from "api/saleOrder";
...
class PopupEffectPackage extends React.Component {
	constructor(props) {
		super(props);
		const {channel_code} = props.branch.currentBranch;
		const configForm = getConfigForm(channel_code, "CustomerCare.SalesOrderEditPage.PackageEffect");

		let object_required = ["service_code", "quantity", "week_quantity", "sku_code", "type_campaign"];
	...
	render() {
		...
		if (box_code.length && box_code[0].service_type === Constant.SERVICE_TYPE_JOB_BASIC) {
			is_basic = true;
			effect_list = effect_list.filter(c => c.code.indexOf(Constant.EFFECT_TYPE_REFRESH_HOUR) >= 0 ||
                c.code.indexOf(Constant.EFFECT_TYPE_REFRESH_DAY) >= 0)
		} else {
			effect_list = effect_list.filter(c => c.detail_part.indexOf(Constant.EFFECT_TYPE_REFRESH_HOUR) < 0 &&
