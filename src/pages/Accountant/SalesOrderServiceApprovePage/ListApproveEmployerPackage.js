...
import PopupServiceReject from "./Popup/PopupServiceReject";

class ListApproveEmployerPackage extends Component {
	constructor(props) {
		super(props);
		this.state = {
			loading: true,
	...
	_refreshList(delay = 0){
		...
		this.props.apiAction.requestApi(apiFn.fnGet, config.apiSalesOrderDomain, ConstantURL.API_URL_GET_ACCOUNT_REGIS_FILTER_RESUME, params, delay);
	}
	_showHide(){
	...
	_btnReject(id){
		this.props.uiAction.createPopup(PopupServiceReject, "Không duyệt gói lọc", {
			object: {registration_filter_resume_id: id},
			refresh_page: "ListApproveEmployerPackage",
			url_reject: ConstantURL.API_URL_POST_ACCOUNT_REGIS_FILTER_RESUME_REJECT,
		});
	}

	componentWillReceiveProps(newProps) {
		if (newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_FILTER_RESUME]){
			let response = newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_FILTER_RESUME];
			...
			this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_GET_ACCOUNT_REGIS_FILTER_RESUME);
		}
		if (newProps.api[ConstantURL.API_URL_GET_EMPLOYER_LIST]){
			let response = newProps.api[ConstantURL.API_URL_GET_EMPLOYER_LIST];
			if(response.info?.args?.service_type === Constant.SERVICE_TYPE_FILTER_RESUME_2018) {
				if (response.code === Constant.CODE_SUCCESS) {
		...
		// if (newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_FILTER_RESUME_APPROVE]) {
		//     let response = newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_FILTER_RESUME_APPROVE];
		//     if (response.code === Constant.CODE_SUCCESS) {
		...
		//     }
		//     this.props.uiAction.hideLoading();
		//     this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_ACCOUNT_REGIS_FILTER_RESUME_APPROVE);
		// }
		if (newProps.refresh["ListApproveEmployerPackage"]){
			let delay = newProps.refresh["ListApproveEmployerPackage"].delay ? newProps.refresh["ListApproveEmployerPackage"].delay : 0;
			this.refreshList(delay);
			this.props.uiAction.deleteRefreshList("ListApproveEmployerPackage");
		}
	}
												...
												<TableBody tableType="TableBody">
													{data_list.map((item, key)=> {
														let data = {
															...
															service: service_list[item.RegistrationFilterResume_service_code] ? service_list[item.RegistrationFilterResume_service_code] : {},
															employer: employer_list[item.employer_id] ? employer_list[item.employer_id] : null,
															registration_status: parseInt(item.RegistrationFilterResume_status),
															registration_id: item.RegistrationFilterResume_id,
															registration_total_point: utils.formatNumber(item.RegistrationFilterResume_total_point,0,".","điểm"),
															registration_total_day_quantity: item.RegistrationFilterResume_total_day_quantity,
															registration_start_date: moment.unix(item.RegistrationFilterResume_start_date).format("DD/MM/YYYY"),
															registration_end_date: moment.unix(item.RegistrationFilterResume_end_date).format("DD/MM/YYYY"),
															registration_created_by: item.RegistrationFilterResume_created_by,
															registration_created_at: moment.unix(item.RegistrationFilterResume_created_at).format("DD/MM/YYYY")
														};
														return(
															<tr key={key} className={classnames("el-table-row", key % 2 !== 0 ? "tr-background" : "")}>
																<td>
																	<div className="cell-custom">
																		...
																		<div>Điểm mua: <span className="text-bold">{data.registration_total_point}</span></div>
																...
																<td>
																	<div className="cell-custom text-underline">
																		{[Constant.STATUS_INACTIVED].includes(data.registration_status) && (
																			<React.Fragment>
																				<div className="text-underline pointer">
																					<span className="text-bold text-success" onClick={()=>{this.btnApprove(data.registration_id)}}>
                                                                                        Duyệt
																					</span>
																				</div>
																				<div className="text-underline pointer">
																					<span className="text-bold text-danger" onClick={()=>{this.btnReject(data.registration_id)}}>
                                                                                        Không duyệt
																					</span>
																				</div>
																			</React.Fragment>
																		...
																		<div className="text-underline pointer">
																			<span className="text-bold text-primary" onClick={()=>{this.btnPreview(data.id)}}>
                                                                                In phiếu
																			</span>
																		</div>
																	</div>
...
export default connect(mapStateToProps,mapDispatchToProps)(ListApproveEmployerPackage);
