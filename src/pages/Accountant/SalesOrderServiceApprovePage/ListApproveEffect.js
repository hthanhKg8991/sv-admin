...
class ListApproveEffect extends Component {
	constructor(props) {
		super(props);
		this.state = {
			loading: true,
	...
	_refreshList(delay = 0){
		...
		this.props.apiAction.requestApi(apiFn.fnGet, config.apiSalesOrderDomain, ConstantURL.API_URL_GET_ACCOUNT_REGIS_EFFECT, params, delay);
	}
	_showHide(){
	...
	//             this.props.uiAction.showLoading();
	//             this.props.apiAction.requestApi(apiFn.fnPost, config.apiSalesOrderDomain, ConstantURL.API_URL_POST_ACCOUNT_REGIS_EFFECT_APPROVE, {registration_effect_id: id});
	//         }
	//     });
	// }
	_btnReject(id){
		this.props.uiAction.createPopup(PopupServiceReject, "Không Duyệt Gói Hiệu Ứng", {
			object: {registration_effect_id: id},
			refresh_page: "ListApproveEffect",
			url_reject: ConstantURL.API_URL_POST_ACCOUNT_REGIS_EFFECT_REJECT,
		});
	}

	componentWillReceiveProps(newProps) {
		if (newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_EFFECT]){
			let response = newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_EFFECT];
			...
			this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_GET_ACCOUNT_REGIS_EFFECT);
		}
		...
		if (newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_EFFECT_APPROVE]) {
			let response = newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_EFFECT_APPROVE];
			if (response.code === Constant.CODE_SUCCESS) {
			...
			this.props.uiAction.hideLoading();
			this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_ACCOUNT_REGIS_EFFECT_APPROVE);
		}
		if (newProps.refresh["ListApproveEffect"]){
			let delay = newProps.refresh["ListApproveEffect"].delay ? newProps.refresh["ListApproveEffect"].delay : 0;
			this.refreshList(delay);
			this.props.uiAction.deleteRefreshList("ListApproveEffect");
		}
	}
											...
											<TableComponent className="table-custom">
												...
												<TableBody tableType="TableBody">
													{data_list.map((item, key)=> {
														let effect_name = "";
														item.RegistrationEffect_detail_part.forEach((item, key) => {
...
export default connect(mapStateToProps,mapDispatchToProps)(ListApproveEffect);
