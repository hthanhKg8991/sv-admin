...
class ComponentFilter extends Component {
	...
	_onSearch(params){
		...
		this.props.uiAction.refreshList("SalesOrderServiceApprovePage");
	}
	_getCustomerCare(){
	...
	render () {
		...
		request_status = request_status.filter(c => parseInt(c.value) !== Constant.STATUS_DELETED);
		return (
			<BoxSearch showQtty={4} onChange={this.onSearch}>
				<SearchField type="input" label="Mã phiếu, mã - tên NTD" name="q" timeOut={1000}/>
				<SearchField type="dropbox" label="Trạng thái" name="status" data={request_status} noDelete/>
