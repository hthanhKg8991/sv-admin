...
class ListApproveJobBox extends Component {
	constructor(props) {
		super(props);
		this.state = {
			loading: true,
	...
	_refreshList(delay = 0){
		...
		this.props.apiAction.requestApi(apiFn.fnGet, config.apiSalesOrderDomain, ConstantURL.API_URL_GET_ACCOUNT_REGIS_JOB_BOX, params, delay);
	}
	_showHide(){
	...
	_btnApprove(id){
		this.props.uiAction.SmartMessageBox({
			...
			if (ButtonPressed === "Yes") {
				this.props.uiAction.showLoading();
				this.props.apiAction.requestApi(apiFn.fnPost, config.apiSalesOrderDomain, ConstantURL.API_URL_POST_ACCOUNT_REGIS_JOB_BOX_APPROVE, {registration_job_box_id: id});
			}
		});
	}
	_btnReject(id){
		this.props.uiAction.createPopup(PopupServiceReject, "Không Duyệt Gói Cơ Bản", {
			object: {registration_job_box_id: id},
			refresh_page: "ListApproveJobBox",
			url_reject: ConstantURL.API_URL_POST_ACCOUNT_REGIS_JOB_BOX_REJECT,
		});
	}

	componentWillReceiveProps(newProps) {
		if (newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_JOB_BOX]){
			let response = newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_JOB_BOX];
			...
			this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_GET_ACCOUNT_REGIS_JOB_BOX);
		}
		...
		// if (newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_JOB_BOX_APPROVE]) {
		//     let response = newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_JOB_BOX_APPROVE];
		//     if (response.code === Constant.CODE_SUCCESS) {
		...
		//     }
		//     this.props.uiAction.hideLoading();
		//     this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_ACCOUNT_REGIS_JOB_BOX_APPROVE);
		// }
		if (newProps.refresh["ListApproveJobBox"]){
			let delay = newProps.refresh["ListApproveJobBox"].delay ? newProps.refresh["ListApproveJobBox"].delay : 0;
			this.refreshList(delay);
			this.props.uiAction.deleteRefreshList("ListApproveJobBox");
		}
	}
			...
			<div className="col-result-full crm-section">
				<div className="box-card box-full">
					<div className="box-card-title pointer card-header" onClick={this.showHide}>
						<span className="title left">Tin phí ({total})</span>
...
export default connect(mapStateToProps,mapDispatchToProps)(ListApproveJobBox);
