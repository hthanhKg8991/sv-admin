...
class ListApproveBanner extends Component {
	constructor(props) {
		super(props);
		this.state = {
			loading: true,
	...
	_refreshList(delay = 0){
		...
		this.props.apiAction.requestApi(apiFn.fnGet, config.apiSalesOrderDomain, ConstantURL.API_URL_GET_ACCOUNT_REGIS_BANNER, params, delay);
	}
	_showHide(){
	...
	_btnApprove(id){
		this.props.uiAction.SmartMessageBox({
			...
			if (ButtonPressed === "Yes") {
				this.props.uiAction.showLoading();
				this.props.apiAction.requestApi(apiFn.fnPost, config.apiSalesOrderDomain, ConstantURL.API_URL_POST_ACCOUNT_REGIS_BANNER_APPROVE, {registration_banner_id: id});
			}
		});
	}
	_btnReject(id){
		this.props.uiAction.createPopup(PopupServiceReject, "Không Duyệt Gói Lọc", {
			object: {registration_banner_id: id},
			refresh_page: "ListApproveBanner",
			url_reject: ConstantURL.API_URL_POST_ACCOUNT_REGIS_BANNER_REJECT,
		});
	}

	componentWillReceiveProps(newProps) {
		if (newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_BANNER]){
			let response = newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_BANNER];
			...
			this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_GET_ACCOUNT_REGIS_BANNER);
		}
		...
		if (newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_BANNER_APPROVE]) {
			let response = newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_BANNER_APPROVE];
			if (response.code === Constant.CODE_SUCCESS) {
			...
			this.props.uiAction.hideLoading();
			this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_ACCOUNT_REGIS_BANNER_APPROVE);
		}
		if (newProps.refresh["ListApproveBanner"]){
			let delay = newProps.refresh["ListApproveBanner"].delay ? newProps.refresh["ListApproveBanner"].delay : 0;
			this.refreshList(delay);
			this.props.uiAction.deleteRefreshList("ListApproveBanner");
		}
	}
			...
			<div className="col-result-full crm-section">
				<div className="box-card box-full">
					<div className="box-card-title pointer card-header" onClick={this.showHide}>
						<span className="title left">Quảng bá thương hiệu ({total})</span>
...
export default connect(mapStateToProps,mapDispatchToProps)(ListApproveBanner);
