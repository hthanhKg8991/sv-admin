...
class ListApproveMinisite extends Component {
	constructor(props) {
		super(props);
		this.state = {
			loading: true,
	...
	_refreshList(delay = 0){
		...
		this.props.apiAction.requestApi(apiFn.fnGet, config.apiSalesOrderDomain, ConstantURL.API_URL_GET_ACCOUNT_REGIS_MINISITE, params, delay);
	}
	_showHide(){
	...
	//         title: "Bạn có chắc muốn duyệt gói lọc?",
	//         content: "",
	//         buttons: ['No','Yes']
	//     }, (ButtonPressed) => {
	//         if (ButtonPressed === "Yes") {
	//             this.props.uiAction.showLoading();
	//             this.props.apiAction.requestApi(apiFn.fnPost, config.apiSalesOrderDomain, ConstantURL.API_URL_POST_ACCOUNT_REGIS_MINISITE_APPROVE, {registration_minisite_id: id});
	//         }
	//     });
	// }
	_btnReject(id){
		this.props.uiAction.createPopup(PopupServiceReject, "Không Duyệt Gói Lọc", {
			object: {registration_minisite_id: id},
			refresh_page: "ListApproveMinisite",
			url_reject: ConstantURL.API_URL_POST_ACCOUNT_REGIS_MINISITE_REJECT,
		});
	}

	componentWillReceiveProps(newProps) {
		if (newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_MINISITE]){
			let response = newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_MINISITE];
			...
			this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_GET_ACCOUNT_REGIS_MINISITE);
		}
		...
		if (newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_MINISITE_APPROVE]) {
			let response = newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_MINISITE_APPROVE];
			if (response.code === Constant.CODE_SUCCESS) {
			...
			this.props.uiAction.hideLoading();
			this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_ACCOUNT_REGIS_MINISITE_APPROVE);
		}
		if (newProps.refresh["ListApproveMinisite"]){
			let delay = newProps.refresh["ListApproveMinisite"].delay ? newProps.refresh["ListApproveMinisite"].delay : 0;
			this.refreshList(delay);
			this.props.uiAction.deleteRefreshList("ListApproveMinisite");
		}
	}
												...
												<TableBody tableType="TableBody">
													{data_list.map((item, key)=> {
														let data = {
															...
															service: service_list[item.RegistrationMinisite_service_code] ? service_list[item.RegistrationMinisite_service_code] : {},
															employer: employer_list[item.employer_id] ? employer_list[item.employer_id] : null,
															registration_status: parseInt(item.RegistrationMinisite_status),
															registration_id: item.RegistrationMinisite_id,
															registration_total_day_quantity: item.RegistrationMinisite_total_day_quantity,
															registration_start_date: moment.unix(item.RegistrationMinisite_start_date).format("DD/MM/YYYY"),
															registration_end_date: moment.unix(item.RegistrationMinisite_end_date).format("DD/MM/YYYY"),
															registration_created_by: item.RegistrationMinisite_created_by,
															registration_created_at: moment.unix(item.RegistrationMinisite_created_at).format("DD/MM/YYYY")
														};
														return(
...
export default connect(mapStateToProps,mapDispatchToProps)(ListApproveMinisite);
