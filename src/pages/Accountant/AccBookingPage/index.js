...
class index extends Component {
	...
	_btnAdd(){
		this.props.uiAction.createPopup(PopupBooking, "Thêm Đặt chổ", {refresh_page: "AccBookingPage"});
	}
	_btnDelete(item){
		this.props.uiAction.SmartMessageBox({
			...
			if (ButtonPressed === "Yes") {
				this.props.uiAction.showLoading();
				this.props.apiAction.requestApi(apiFn.fnPost, config.apiBookingDomain, ConstantURL.API_URL_POST_BOOKING_DELETE, {code: item.code});
			}
		});
	}
	...
	componentWillReceiveProps(newProps) {
		...
		if (newProps.api[ConstantURL.API_URL_POST_BOOKING_DELETE]) {
			let response = newProps.api[ConstantURL.API_URL_POST_BOOKING_DELETE];
			if (response.code === Constant.CODE_SUCCESS) {
				this.props.uiAction.hideSmartMessageBox();
				this.props.uiAction.putToastSuccess("Thao tác thành công!");
				this.refreshList();
			}
			this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_BOOKING_DELETE);
		}
		if (newProps.refresh["AccBookingPage"]){
			let delay = newProps.refresh["AccBookingPage"].delay ? newProps.refresh["AccBookingPage"].delay : 0;
			this.refreshList(delay);
			this.props.uiAction.deleteRefreshList("AccBookingPage");
		}
		...
		return (
			<div className="row-body">
				<div className="col-search">
					<CustomFilter name="AccBookingPage" />
