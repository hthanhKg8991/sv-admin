...
class PopupBooking extends Component {
	constructor(props) {
		super(props);
		this.state = {
			object: {},
	...
	_onSave(object_input, object_required) {
		...
		this.props.apiAction.requestApi(
			apiFn.fnPost,
			config.apiBookingDomain,
			ConstantURL.API_URL_POST_BOOKING_CREATE,
			object
		);
	}
	...
	_getBoxList() {
		this.props.apiAction.requestApi(apiFn.fnGet, config.apiBookingDomain, ConstantURL.API_URL_GET_LIST_BOX, {});
	}
	_getCustomerCare() {
	...
	componentWillReceiveProps(newProps) {
		...
		if (newProps.api[ConstantURL.API_URL_POST_BOOKING_CREATE]) {
			let response = newProps.api[ConstantURL.API_URL_POST_BOOKING_CREATE];
			if (response.code === Constant.CODE_SUCCESS) {
				this.props.uiAction.deletePopup();
				this.props.uiAction.putToastSuccess("Thao tác thành công!");
				this.props.uiAction.refreshList(this.props.refresh_page);
			} else {
				this.setState({ object_error: Object.assign({}, response.data) });
			}
			this.props.uiAction.hideLoading();
			this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_BOOKING_CREATE);
		}
	}
	...
	render() {
		...
		let service_page_type = utils.convertArrayValueCommonData(
			this.props.sys.common.items,
			Constant.COMMON_DATA_KEY_service_page_type
		);
		...
		let box = box_list.filter(
			(c) => parseInt(c.page_type_id) === parseInt(object.page_type_id) && c.gate_code === object.gate
		);
		...
		if (this.props.refresh_page === "BookingPage") {
			staff_list = staff_list.filter((c) => String(c.id) === String(this.props.user.id));
		}
		return (
			<form
				onSubmit={(event) => {
					...
					<div className="popupContainer">
						<div className="form-container row">
							<div className="col-sm-12 col-xs-12 padding0">
								...
								<div className="col-sm-6 col-xs-12 mb10">
									<Dropbox
										name="page_type_id"
										label="Loại trang"
										data={service_page_type}
										required={object_required.includes("page_type_id")}
										value={object.page_type_id}
										error={object_error.page_type_id}
										nameFocus={name_focus}
										onChange={this.onChange}
									/>
								</div>
							...
							<div className="col-sm-12 col-xs-12 padding0">
								...
								{object_required.includes("job_field_id") && (
									<div className="col-sm-6 col-xs-12 mb10">
										<Dropbox
											name="job_field_id"
											label="Ngành"
											...
											key_value="id"
											key_title="name"
											value={object.job_field_id}
											error={object_error.job_field_id}
											nameFocus={name_focus}
											onChange={this.onChange}
										/>
									</div>
							...
							<div className="col-sm-12 col-xs-12 padding0">
								...
								<div className="col-sm-6 col-xs-12 mb10">
									<Dropbox
										...
										data={display_method}
										required={object_required.includes("display_method")}
										error={object_error.display_method}
										value={object.display_method}
										nameFocus={name_focus}
										onChange={this.onChange}
									/>
								</div>
							...
							<div className="col-sm-12 col-xs-12 padding0">
								<div className="col-sm-6 col-xs-12 mb10">
									<DateTimePicker
										name="from_date"
										label="Ngày bắt đầu"
										required={object_required.includes("from_date")}
										minDate={moment()}
										error={object_error.from_date}
								...
								<div className="col-sm-6 col-xs-12 mb10">
									<DateTimePicker
										name="to_date"
										label="Ngày kết thúc"
										required={object_required.includes("to_date")}
										minDate={moment()}
										error={object_error.to_date}
...
export default connect(mapStateToProps, mapDispatchToProps)(PopupBooking);
