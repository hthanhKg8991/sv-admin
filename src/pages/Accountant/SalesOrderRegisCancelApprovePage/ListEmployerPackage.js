...
class ListEmployerPackage extends Component {
	constructor(props) {
		super(props);
		this.state = {
			loading: true,
	...
	_refreshList(delay = 0){
		...
		this.props.apiAction.requestApi(apiFn.fnGet, config.apiSalesOrderDomain, ConstantURL.API_URL_GET_ACCOUNT_REGIS_CANCEL_FILTER_RESUME_LIST, params, delay);
	}
	_showHide(){
	...
	//             this.props.uiAction.showLoading();
	//             this.props.apiAction.requestApi(apiFn.fnPost, config.apiSalesOrderDomain, ConstantURL.API_URL_POST_ACCOUNT_REGIS_CANCEL_FILTER_RESUME_APPROVE, {
	...
	// }
	_btnReject(item){
		this.props.uiAction.createPopup(PopupDeleteRegisCancel, "Không Duyệt Gói Điểm Dịch Vụ", {
			object: {
				drop_registration_id: item.SalesOrderRequestDropRegistration_id
			},
			refresh_page: "ListEmployerPackage",
			url_reject: ConstantURL.API_URL_POST_ACCOUNT_REGIS_FILTER_RESUME_REJECT,
		});
	}

	componentWillReceiveProps(newProps) {
		if (newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_CANCEL_FILTER_RESUME_LIST]){
			let response = newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_CANCEL_FILTER_RESUME_LIST];
			...
			this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_GET_ACCOUNT_REGIS_CANCEL_FILTER_RESUME_LIST);
		}
		...
		// if (newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_CANCEL_FILTER_RESUME_APPROVE]) {
		//     let response = newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_CANCEL_FILTER_RESUME_APPROVE];
		//     if (response.code === Constant.CODE_SUCCESS) {
		...
		//     }
		//     this.props.uiAction.hideLoading();
		//     this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_ACCOUNT_REGIS_CANCEL_FILTER_RESUME_APPROVE);
		// }
		if (newProps.refresh["ListEmployerPackage"]){
			let delay = newProps.refresh["ListEmployerPackage"].delay ? newProps.refresh["ListEmployerPackage"].delay : 0;
			this.refreshList(delay);
			this.props.uiAction.deleteRefreshList("ListEmployerPackage");
		}
	}
			...
			<div className="col-result-full crm-section">
				<div className="box-card box-full">
					<div className="box-card-title pointer card-header" onClick={this.showHide}>
						<span className="title left">Yêu Cầu Hạ Gói Điểm Dịch Vụ ({total})</span>
...
export default connect(mapStateToProps,mapDispatchToProps)(ListEmployerPackage);
