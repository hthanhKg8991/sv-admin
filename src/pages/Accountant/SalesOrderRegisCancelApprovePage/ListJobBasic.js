...
class ListJobBasic extends Component {
	constructor(props) {
		super(props);
		this.state = {
			loading: true,
	...
	_refreshList(delay = 0){
		...
		this.props.apiAction.requestApi(apiFn.fnGet, config.apiSalesOrderDomain, ConstantURL.API_URL_GET_ACCOUNT_REGIS_CANCEL_JOB_BASIC_LIST, params, delay);
	}
	_showHide(){
	...
	_btnReject(item){
		this.props.uiAction.createPopup(PopupDeleteRegisCancel, "Không Duyệt Gói Cơ Bản", {
			object: {
				drop_registration_id: item.SalesOrderRequestDropRegistration_id
			},
			refresh_page: "ListJobBasic",
			url_reject: ConstantURL.API_URL_POST_ACCOUNT_REGIS_CANCEL_JOB_BASIC_REJECT,
		});
	}

	componentWillReceiveProps(newProps) {
		if (newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_CANCEL_JOB_BASIC_LIST]){
			let response = newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_CANCEL_JOB_BASIC_LIST];
			...
			this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_GET_ACCOUNT_REGIS_CANCEL_JOB_BASIC_LIST);
		}
		...
		if (newProps.refresh["ListJobBasic"]){
			let delay = newProps.refresh["ListJobBasic"].delay ? newProps.refresh["ListJobBasic"].delay : 0;
			this.refreshList(delay);
			this.props.uiAction.deleteRefreshList("ListJobBasic");
		}
	}
			...
			<div className="col-result-full crm-section">
				<div className="box-card box-full">
					<div className="box-card-title pointer card-header" onClick={this.showHide}>
						<span className="title left">Yêu Cầu Hạ Tin Cơ Bản ({total})</span>
...
export default connect(mapStateToProps,mapDispatchToProps)(ListJobBasic);
