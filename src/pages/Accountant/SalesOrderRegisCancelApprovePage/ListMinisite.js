...
class ListMinisite extends Component {
	constructor(props) {
		super(props);
		this.state = {
			loading: true,
	...
	_refreshList(delay = 0){
		...
		this.props.apiAction.requestApi(apiFn.fnGet, config.apiSalesOrderDomain, ConstantURL.API_URL_GET_ACCOUNT_REGIS_CANCEL_MINISITE_LIST, params, delay);
	}
	_showHide(){
	...
	_btnApprove(item){
		this.props.uiAction.SmartMessageBox({
			title: "Bạn có chắc muốn duyệt minisite ?",
			content: "",
			buttons: ["No","Yes"]
		}, (ButtonPressed) => {
	...
	_btnReject(item){
		this.props.uiAction.createPopup(PopupDeleteRegisCancel, "Không Duyệt Minisite", {
			object: {
				drop_registration_id: item.SalesOrderRequestDropRegistration_id
			},
			refresh_page: "ListMinisite",
			url_reject: ConstantURL.API_URL_POST_ACCOUNT_REGIS_CANCEL_MINISITE_REJECT,
		});
	}

	componentWillReceiveProps(newProps) {
		if (newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_CANCEL_MINISITE_LIST]){
			let response = newProps.api[ConstantURL.API_URL_GET_ACCOUNT_REGIS_CANCEL_MINISITE_LIST];
			...
			this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_GET_ACCOUNT_REGIS_CANCEL_MINISITE_LIST);
		}
		...
		if (newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_CANCEL_MINISITE_APPROVE]) {
			let response = newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_CANCEL_MINISITE_APPROVE];
			if (response.code === Constant.CODE_SUCCESS) {
			...
			this.props.uiAction.hideLoading();
			this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_ACCOUNT_REGIS_CANCEL_MINISITE_APPROVE);
		}
		if (newProps.refresh["ListMinisite"]){
			let delay = newProps.refresh["ListMinisite"].delay ? newProps.refresh["ListMinisite"].delay : 0;
			this.refreshList(delay);
			this.props.uiAction.deleteRefreshList("ListMinisite");
		}
	}
			...
			<div className="col-result-full crm-section">
				<div className="box-card box-full">
					<div className="box-card-title pointer card-header" onClick={this.showHide}>
						<span className="title left">Yêu cầu hạ Minisite ({total})</span>
...
export default connect(mapStateToProps,mapDispatchToProps)(ListMinisite);
