...
class ListBanner extends Component {
	...
	_btnApprove(item){
		this.props.uiAction.SmartMessageBox({
			title: "Bạn có chắc muốn duyệt banner ?",
			content: "",
			buttons: ["No","Yes"]
		}, (ButtonPressed) => {
			if (ButtonPressed === "Yes") {
				this.props.uiAction.showLoading();
				this.props.apiAction.requestApi(apiFn.fnPost, config.apiSalesOrderDomain, ConstantURL.API_URL_POST_ACCOUNT_REGIS_CANCEL_BANNER_APPROVE, {
	...
	_btnReject(item){
		this.props.uiAction.createPopup(PopupDeleteRegisCancel, "Không Duyệt Quảng bá thương hiệu", {
			object: {
				drop_registration_id: item.SalesOrderRequestDropRegistration_id
			},
			refresh_page: "ListBanner",
			url_reject: ConstantURL.API_URL_POST_ACCOUNT_REGIS_CANCEL_BANNER_REJECT,
		});
	}
	...
	componentWillReceiveProps(newProps) {
		...
		if (newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_CANCEL_BANNER_APPROVE]) {
			let response = newProps.api[ConstantURL.API_URL_POST_ACCOUNT_REGIS_CANCEL_BANNER_APPROVE];
			if (response.code === Constant.CODE_SUCCESS) {
			...
			this.props.uiAction.hideLoading();
			this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_ACCOUNT_REGIS_CANCEL_BANNER_APPROVE);
		}
			...
			<div className="col-result-full crm-section">
				<div className="box-card box-full">
					<div className="box-card-title pointer card-header" onClick={this.showHide}>
						<span className="title left">Yêu cầu hạ Quảng bá thương hiệu ({total})</span>
