...
import {deleteItemsGroup, getListItemsGroup, updateItemsGroup} from "api/saleOrder";
...
class List extends Component {
			...
			columns: [
				...
				},
				{
					...
					cell: row => (
                        ...
                        <>
                        	<CanRender actionCode={ROLES.accountant_accountant_product_group_update}>
                        		<span className="text-link text-blue font-bold" onClick={() => this.onEdit(row?.id)}>Chỉnh sửa</span>
                        	</CanRender> <br/>
                        	{
                        		row?.status === Constant.STATUS_ACTIVED ?
                        			<CanRender actionCode={ROLES.accountant_accountant_product_group_change_status}>
                        	...
                        	{
                        		row?.status !== Constant.STATUS_ACTIVED &&
                                <CanRender actionCode={ROLES.accountant_accountant_product_group_delete}>
	...
	async _onChangeStatus(id, status) {
		...
		const res = await updateItemsGroup({
			status: statusChange,
			id
		});
		if (res) {
	...
	_onDelete(id) {
		...
		actions.SmartMessageBox({
			...
			if (ButtonPressed === "Yes") {
				const res = await deleteItemsGroup({id});
				if (res) {
					actions.putToastSuccess("Thao tác thành công");
					publish(".refresh", {}, idKey);
				}
			...
			<Default
				...
				buttons={(
					<div className="left btnCreateNTD">
						<CanRender actionCode={ROLES.accountant_accountant_product_group_create}>
