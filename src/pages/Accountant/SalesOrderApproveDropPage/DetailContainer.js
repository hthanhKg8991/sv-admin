...
import Detail from "pages/Accountant/SalesOrderApproveDropPage/Detail";
...
class DetailContainer extends Component {
	...
	async _getDetailSalesOrder() {
		const {id} = this.state;
		return await getDetailSalesOrder({ id, type: Constant.SALES_ORDER_TYPE });
	}

	componentDidMount() {
		const resDetail = this._getDetailSalesOrder();
		resDetail.then((data) => {
			this.setState({
				saleOrder: data
			})
	...
	render() {
		...
		const idKey = "SalesOrderApproveDropDetail";
