...
import {deleteSubscription, getListSubscription, toggleActiveSubscription} from "api/saleOrder";
...
import ComponentFilter from "pages/Accountant/SubscriptionPackagePage/ComponentFilter";

const idKey = "SubscriptionPackagePageList";
...
class List extends Component {
	constructor(props) {
		...
		this.state = {
			columns: [
				...
				{
					title: "Mã Subscription",
					width: 120,
					accessor: "code"
				},
				{
					title: "Tên Subscription",
					width: 100,
					accessor: "name"
				},
				{
                        ...
                        <>
                        	...
                        	</CanRender> <br/>
                        	{
                        		row?.status === Constant.STATUS_ACTIVED ?
                        			<CanRender actionCode={ROLES.accountant_subscription_package_toggle_active}>
	...
	async _onChangeStatus(id) {
		...
		const res = await toggleActiveSubscription({
			id
		});
		if (res) {
	...
	render() {
		...
		return (
			<Default
				...
				title="Danh Sách Các Gói Subscription"
				titleActions={(
