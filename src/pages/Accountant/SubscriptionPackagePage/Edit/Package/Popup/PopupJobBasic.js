...
import { createSubscriptionItem, getDetailSubscription } from "api/saleOrder";
...
class PopupJobBasic extends Component {
	...
	async _onSave(data) {
		...
		if (parseInt(object.quantity) < -1) {
			error["quantity"] = ":attr_name không hợp lệ.";
		}
		if (parseInt(object.based_price) <= 0 && this.state.subsType === 3){
			error["based_price"] = ":attr_name là bắt buộc.";
		}
	...
	async _asyncData(){
		...
		const {id} = this.state;
		if (id > 0) {
			const resSubsDetail = await getDetailSubscription({id});
			if (resSubsDetail) {
				this.setState({subsType: resSubsDetail?.type_campaign});
				if(resSubsDetail?.type_campaign === 3){
					this.setState({object_required: [...object_required, "based_price"]});
				}
			}
		}
	}
	...
	render() {
		...
		const {object, object_error, object_required, name_focus, subsType} = this.state;
					...
					<div className="popupContainer">
						<div className="form-container row">
							...
							<div className="col-sm-12 col-xs-12 padding0">
								...
								{subsType === 3 && <div className="col-sm-6 col-xs-12 mb10">
									<Input2 type="text" name="based_price" label="Thành tiền" isNumber
										suffix=" VND"
										error={object_error.based_price} value={object.based_price}
										required={object_required.includes("based_price")}
										nameFocus={name_focus}
										onChange={this.onChange}
									/>
								</div>}
