...
class PopupFreemium extends Component {
  constructor(props) {
    super(props);
    const { channel_code } = props.branch.currentBranch;
    const configForm = getConfigForm(
        channel_code,
        "CustomerCare.SalesOrderEditPage.PackageJobBasic"
    );
    const isGift =
        props.type_campaign === Constant.SALES_ORDER_ITEM_TYPE_CAMPAIGN.gift;
    const weekQuantity =
        isGift || configForm.includes("empty_week_quantity")
            ? null
            : Constant.JOB_BASIC_WEEK_QUANTITY_DEFAULT;
  ...
  async _onSave(data) {
    ...
    if (
        parseInt(object.week_quantity) >
        Constant.WEEK_QUANTITY_MAX_JOB_BASIC[channel_code]
    ) {
      error[
          "week_quantity"
          ] = `:attr_name quá gới hạn (${Constant.WEEK_QUANTITY_MAX_JOB_BASIC[channel_code]} tuần).`;
    }
    ...
    object.service_type = Constant.SERVICE_TYPE_JOB_FREEMIUM;
  ...
  componentDidMount() {
    const box_code_list = this.props.sys.service.items.filter(
        (c) => c.service_type === Constant.SERVICE_TYPE_JOB_FREEMIUM
    );
  ...
  render() {
    ...
    const box_code_list = this.props.sys.service.items.filter(
        (c) =>
            ...
            c.service_type === Constant.SERVICE_TYPE_JOB_FREEMIUM
    );
    const end_date = utils.calcDate(
        object.start_date,
...
export default connect(mapStateToProps, mapDispatchToProps)(PopupFreemium);
