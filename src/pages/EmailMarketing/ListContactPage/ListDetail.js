...
import ComponentFilter from "pages/EmailMarketing/ListContactPage/ComponentFilterDetail";
...
import PopupForm from "pages/EmailMarketing/ListContactPage/Popup/PopupDetailForm";
import * as Constant from "utils/Constant";
import { createByImportEmailMarketing, deleteListContactDetail, getListListContactDetail } from "api/emailMarketing";
import SpanCommon from "components/Common/Ui/SpanCommon";
import config from "config";
const idKey = "EmailMarketingContactDetailList";
...
class List extends Component {
    ...
    this.state = {
      columns: [
        ...
        },
        {
          title: "Lọai",
          width: 100,
          cell: (row) => <SpanCommon idKey={Constant.COMMON_DATA_KEY_email_marketing_list_contact_type} value={row.type} />,
        },
        {
          title: "Hành động",
          width: 100,
  ...
  _onClickAddSegment() {
    ...
    history.push({
      ...
      search: "?action=addSegment&list_contact_id=" + list_contact_id,
    });
  }
  ...
  _onDownFileSample() {
    window.open(`${config.apiEmailMarketingDomain}${Constant.EMAIL_MARKETING_LIST_CONTACT_FILE_IMPORT_SAMPLE}`);
  }
  ...
  async _onChangeFileImport(event) {
    ...
    const res = await createByImportEmailMarketing(body);
    if (res) {
            ...
            <div className="left btnCreateNTD">
              ...
              <CanRender actionCode={ROLES.email_marketing_list_contact_import_list}>
                <label className="el-button el-button-primary el-button-small">
                  <span>
                    Import danh sách <i className="glyphicon glyphicon-plus" />
                  </span>
                  <input
                    type="file"
                    className="hidden"
                ...
                <button type="button" className="el-button el-button-primary el-button-small" onClick={this.onDownFileSample}>
                  <span>
                    Tải file import mẫu <i className="glyphicon glyphicon-download" />
                  </span>
                </button>
              </CanRender>
              <div>
