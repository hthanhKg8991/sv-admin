...
import {
    createFieldPrintTemplate,
    getDetailFieldPrintTemplate,
    updateFieldPrintTemplate,
} from "api/saleOrder";
...
class Edit extends React.Component {
    ...
    async submitData(data, setErrors) {
        ...
        if (id > 0) {
            data.id = id;
            res = await updateFieldPrintTemplate(data);
        } else {
            res = await createFieldPrintTemplate(data);
        }
        if (res) {
    ...
    async asyncData() {
        ...
        if (id > 0) {
            const res = await getDetailFieldPrintTemplate({id});
            if (res) {
    ...
    render() {
        ...
        const validationSchema = Yup.object().shape({
            ...
            template: Yup.string().required(Constant.MSG_REQUIRED),
            name_representative: Yup.string().required(Constant.MSG_REQUIRED),
            position_representative: Yup.string().required(Constant.MSG_REQUIRED),
            phone_representative: Yup.string().required(Constant.MSG_REQUIRED),
            address_representative: Yup.string().required(Constant.MSG_REQUIRED),
