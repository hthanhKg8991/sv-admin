...
import ComponentFilter from "pages/Checkmate/FieldPriceListPage/ComponentFilter";
import {bindActionCreators} from 'redux';
import {hideSmartMessageBox, putToastError, putToastSuccess, SmartMessageBox} from "actions/uiAction";
import CanRender from "components/Common/Ui/CanRender";
import ROLES from "utils/ConstantActionCode";
import {
    getListFieldPriceList,
    deleteFieldPriceList,
    approveFieldPriceList
} from "api/saleOrder";
import SpanCommon from "components/Common/Ui/SpanCommon";

const idKey = "AudienceList";
...
class List extends Component {
        ...
        this.state = {
            columns: [
                ...
                },
                {
                    ...
                    cell: row => (
                        <>
                            <CanRender actionCode={ROLES.sales_order_by_field_field_price_list_update}>
                            ...
                            {parseInt(row?.status) !== Constant.STATUS_ACTIVED && (
                                <>
                                    <CanRender actionCode={ROLES.sales_order_by_field_field_price_list_approve}>
                                       <span className="text-link text-success font-bold ml10"
                                             onClick={() => this.onApprove(row?.id)}>Duyệt</span>
                                    </CanRender>
                                    <CanRender actionCode={ROLES.sales_order_by_field_field_price_list_delete}>
    ...
    _onApprove(id) {
        ...
        actions.SmartMessageBox({
            ...
            if (ButtonPressed === "Yes") {
                const res = await approveFieldPriceList({id});
                if (res) {
    ...
    _onDelete(id) {
        ...
        actions.SmartMessageBox({
            ...
            if (ButtonPressed === "Yes") {
                const res = await deleteFieldPriceList({id});
                if (res) {
                    actions.putToastSuccess('Thao tác thành công');
                    publish(".refresh", {}, idKey);
                }
            ...
            <Default
                ...
                buttons={(
                    <div className="left btnCreateNTD">
                        <CanRender actionCode={ROLES.sales_order_by_field_field_price_list_create}>
                ...
                <Gird idKey={idKey}
                      fetchApi={getListFieldPriceList}
                      query={query}
                      columns={columns}
