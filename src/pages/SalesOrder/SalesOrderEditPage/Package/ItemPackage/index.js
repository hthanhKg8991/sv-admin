...
import {
    getListFullCategory,
    getListFullSalesOrderItemV2,
    postDeleteSalesOrderItemV2,
    postUpdateSalesOrderItemV2,
    postUpdateSalesOrderV2
} from "api/saleOrderV2";
import PopupItemPackage from "pages/SalesOrder/SalesOrderEditPage/Popup/PopupItemPackage";
...
class index extends Component {
    ...
    async _onChangeDiscount(item) {
        ...
        const res = await postUpdateSalesOrderItemV2({...item});
        if (res) {
    ...
    async _onChangeTotalDiscount(sales_order) {
        const {idKeySalesOrder} = this.props
        const res = await postUpdateSalesOrderV2({...sales_order});
        if (res) {
            this.props.uiAction.putToastSuccess("Thao tác thành công");
            publish(".refresh", {}, idKeySalesOrder);
    ...
    async _btnDelete(id) {
        ...
        uiAction.SmartMessageBox({
            ...
            if (ButtonPressed === "Yes") {
                ...
                const res = await postDeleteSalesOrderItemV2({id});
                if (res) {
    ...
    async _refreshList() {
        ...
        const resListDetail = await getListFullSalesOrderItemV2(args)
        if (resListDetail) {
    ...
    async _asyncData() {
        const resCategory = await getListFullCategory({status: Constant.CATEGORY_STATUS_ACTIVE});
        if (resCategory) {
                    ...
                    return (
                        <>
                            <div className="text-underline pointer mt10 mb10">
                                <span className="text-bold text-primary" onClick={() => {
                                    this.btnDuplicate(item)
                                }}>Sao chép</span>
                            </div>
                        ...
                        <div>
                            {this.state.loading ? (
                                ...
                                <div className="card-body">
                                    <div className="left">
                                        {canEditItem && (
                                            <CanRender actionCode={ROLES.sales_order_sales_order_create_item}>
                                                <button type="button"
