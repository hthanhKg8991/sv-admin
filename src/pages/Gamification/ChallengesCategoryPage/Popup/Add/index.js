...
import {
    createGamificationChallengesCategory,
    updateGamificationChallengesCategory
} from "api/gamification";

class PopupAddChallengesCategory extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            initialForm: {
    ...
    async submitData(data,setErrors) {
        ...
        if (id > 0) {
            data.id = id;
            res = await updateGamificationChallengesCategory(data);
        } else {
            res = await createGamificationChallengesCategory(data);
        }
        if (res) {
...
export default connect(null, mapDispatchToProps)(PopupAddChallengesCategory);
