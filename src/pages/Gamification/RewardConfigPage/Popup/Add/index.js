...
import {
    createGamificationRewardConfig,
    updateGamificationRewardConfig,

} from "api/gamification";

class PopupAddReward extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            initialForm: {
    ...
    async submitData(data) {
        ...
        if (id > 0) {
            data.id = id;
            res = await updateGamificationRewardConfig(data);
        } else {
            res = await createGamificationRewardConfig(data);
        }
        if (res) {
    ...
    render() {
        ...
        const validationSchema = Yup.object().shape({
            ...
            weight: Yup.string().required(Constant.MSG_REQUIRED),
            percent: Yup.string().required(Constant.MSG_REQUIRED).max(100,"Tối đa 100"),
        });
...
export default connect(null, mapDispatchToProps)(PopupAddReward);
