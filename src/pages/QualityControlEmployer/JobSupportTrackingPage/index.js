...
class index extends Component {
    ...
    _refreshList(delay = 0){
        ...
        this.props.apiAction.requestApi(apiFn.fnGet, config.apiEmployerDomain, ConstantURL.API_URL_JOB_SUPPORT_TRACKING_LIST, params, delay);
    ...
    componentWillReceiveProps(newProps) {
        if (newProps.api[ConstantURL.API_URL_JOB_SUPPORT_TRACKING_LIST]) {
            let response = newProps.api[ConstantURL.API_URL_JOB_SUPPORT_TRACKING_LIST];
            if (response.code === Constant.CODE_SUCCESS) {
                this.setState({data_list: response.data.items});
                this.setState({pagination_data: response.data});
            }
            this.props.uiAction.hideLoading();
            this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_JOB_SUPPORT_TRACKING_LIST);
        }
        if (newProps.refresh['JobSupportTrackingPage']){
            let delay = newProps.refresh['JobSupportTrackingPage'].delay ? newProps.refresh['JobSupportTrackingPage'].delay : 0;
            this.refreshList(delay);
            this.props.uiAction.deleteRefreshList('JobSupportTrackingPage');
        }
    ...
    render () {
        ...
        let job_support_tracking_type = utils.convertObjectValueCommonData(this.props.sys.common.items,Constant.COMMON_DATA_KEY_job_support_tracking_type);
        return (
            <div className="row-body">
                <div className="col-search">
                    <CustomFilter name="JobSupportTrackingPage"/>
                    <ComponentFilter history={this.props.history}/>
                </div>
                <div className="col-result">
                    <div className="box-card">
                        <div className="box-card-title">
                            <span className="title left">Dữ Liệu Theo Dõi Hiệu Quả</span>
                            <div className="right">
                                            ...
                                            {data_list.map((item, key)=> {
                                                return (
                                                    <tr key={key} className={classnames("el-table-row pointer", (key % 2 !== 0 ? "tr-background" : ""))}>
                                                        ...
                                                        <td>
                                                            <div className="cell">{item.seeker_id}</div>
                                                        </td>
                                                        <td>
                                                            <div className="cell">{job_support_tracking_type[item.tracking_type]}</div>
                                                        </td>
                                                        <td>
                                                            <div className="cell">{item.template_number}</div>
                                                        </td>
                                                        <td>
