...
import PopupBlockKeyword from "./Popup/PopupBlockKeyword";
...
class index extends Component {
    ...
    _refreshList(delay = 0){
        ...
        this.props.apiAction.requestApi(apiFn.fnGet, config.apiSystemDomain, ConstantURL.API_URL_GET_BLACKLIST, params, delay);
    ...
    _btnAdd(){
        this.props.uiAction.createPopup(PopupBlockKeyword, "Thêm Từ Khóa Cấm");
    }
    _btnDelete(item){
        this.props.uiAction.SmartMessageBox({
            title: "Bạn có chắc muốn xóa từ khóa ?",
            content: "",
            buttons: ['No','Yes']
        }, (ButtonPressed) => {
            if (ButtonPressed === "Yes") {
                this.props.uiAction.showLoading();
                this.props.apiAction.requestApi(apiFn.fnPost, config.apiSystemDomain, ConstantURL.API_URL_POST_BLACKLIST_DELETE, {blacklist_keyword_id: item.id});
            }
        });
    }
    _activeItem(key){
    ...
    componentWillReceiveProps(newProps) {
        if (newProps.api[ConstantURL.API_URL_GET_BLACKLIST]) {
            let response = newProps.api[ConstantURL.API_URL_GET_BLACKLIST];
            ...
            this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_GET_BLACKLIST);
        }
        if (newProps.api[ConstantURL.API_URL_POST_BLACKLIST_DELETE]) {
            let response = newProps.api[ConstantURL.API_URL_POST_BLACKLIST_DELETE];
            if (response.code === Constant.CODE_SUCCESS) {
            ...
            this.props.uiAction.hideLoading();
            this.props.apiAction.deleteRequestApi(ConstantURL.API_URL_POST_BLACKLIST_DELETE);
        }
        if (newProps.refresh['BlockKeywordPage']){
            let delay = newProps.refresh['BlockKeywordPage'].delay ? newProps.refresh['BlockKeywordPage'].delay : 0;
            this.refreshList(delay);
            this.props.uiAction.deleteRefreshList('BlockKeywordPage');
        }
        ...
        return (
            <div className="row-body">
                <div className="col-search">
                    <CustomFilter name="BlockKeywordPage"/>
                    <ComponentFilter history={this.props.history}/>
                </div>
                <div className="col-result">
                    <div className="box-card">
                        <div className="box-card-title">
                            <span className="title left">Danh sách từ khóa cấm</span>
                            <div className="right">
                                ...
                                <div className="top-table">
                                    <div className="left btnCreateNTD">
                                        <button type="button" className="el-button el-button-primary el-button-small" onClick={this.btnAdd}>
                                            <span>Thêm Từ Khóa Cấm <i className="glyphicon glyphicon-plus"/></span>
                                        </button>
                                    </div>
                                </div>
                                                    ...
                                                    <React.Fragment key={key}>
                                                        <tr className={classnames("el-table-row pointer", key % 2 !== 0 ? "tr-background" : "", String(itemActive) === String(item.id) ? "active" : "")}>
                                                            <td onClick={()=>{this.activeItem(item.id)}}>
                                                                <div className="cell" title={item.title}>{item.title}</div>
                                                            </td>
                                                            <td onClick={()=>{this.activeItem(item.id)}}>
                                                                <div className="cell" title={item.slug}>{item.slug}</div>
                                                            </td>
                                                            <td>
                                                                <div className="cell">
