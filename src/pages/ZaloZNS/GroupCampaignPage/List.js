...
import {activeGroupCampaign, deleteGroupCampaign, getListGroupCampaign} from "api/zalo";
import SpanCommon from "components/Common/Ui/SpanCommon";

const idKey = "ZaloZnsGroupCampaignList";
...
class List extends Component {
        ...
        this.state = {
            columns: [
                ...
                },
                {
                    title: "message/tháng",
                    width: 50,
                    accessor: "limit_quota"
                },
                {
                    title: "Trạng thái",
                    width: 120,
                    cell: row =>
                        <SpanCommon idKey={Constant.COMMON_DATA_KEY_zalo_zns_group_campaign_status}
                                    value={row?.status}/>,
                },
                {
                    title: "Hành động",
                    ...
                    cell: row => (
                        <>
                            <CanRender actionCode={ROLES.zalo_zns_group_campaign_update}>
                                <span className="text-link text-blue font-bold ml5" onClick={() => this.onEdit(row)}>
                                    Chỉnh sửa
                                </span>
                            </CanRender>
                            <CanRender actionCode={ROLES.zalo_zns_group_campaign_active}>
                            ...
                            <CanRender actionCode={ROLES.zalo_zns_group_campaign_delete}>
                                {row.status === Constant.STATUS_INACTIVED && (
    ...
    _onActive(id) {
        ...
        actions.SmartMessageBox({
            ...
            actions.hideSmartMessageBox();
            if (ButtonPressed === "Yes") {
                const res = await activeGroupCampaign({id});
                if (res) {
                    actions.putToastSuccess('Thao tác thành công');
                }
                publish(".refresh", {}, idKey);
            ...
            <Default
                ...
                buttons={(
                    <div className="left btnCreateNTD">
                        <CanRender actionCode={ROLES.zalo_zns_group_campaign_create}>
